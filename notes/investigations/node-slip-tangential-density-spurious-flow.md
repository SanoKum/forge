...
- **再構成次数は無関係**: 同一バイナリ・cfl_pseudo 5 で MUSCL (`run_isoT_condL_node_m5`) vs
  1 次風上 (`run_isoT_condL_node_1st`) を A/B → 側面列 max|U| 0.476 vs 0.446 m/s・符号反転率
  0.48 vs 0.41 でほぼ不変。**市松は MUSCL 境界再構成ではなく slip 閉包 (bvar 構成 or 境界半割面
  流束) 自体に起因**する。
- 壁ノード T ピン導入後バイナリでも再現 (slip 列は壁ノードでないため無関係、想定どおり)。
