...
forge (case/48) の反応 ON 計算が実験 H/d≈10 に対し**リップ付着**となり PaSR `C_mix` にも非感応である件について、「どのくらい合うべきなのか」「合わせるにはどのモデルが要るのか」を文献調査した。結論: **平均反応速度 (laminar-chemistry) クロージャの付着火炎は文献で立証済みの想定内失敗モード**であり、ソルバのバグを示すものではない。H/d≈10 を再現した実績があるのは着火遅れ統計を運ぶクロージャ (transported PDF / CMC) のみ。
