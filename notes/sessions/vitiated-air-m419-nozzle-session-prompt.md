...
| 全温 $T_t$ | 1060 K |
| 出口径 | **1600 mm** (半径 0.800 m) ← 2026-08-18 ユーザ訂正 (当初 800 mm と伝えられたが半径だった) |
| 目標出口マッハ数 $M_d$ | **4.19** |
...
**入口配管径**: ユーザ指定 (2026-08-18) で**出口径と同じ φ1600 mm** → `geometry.r_inlet: 3.8`
(=3.8004 $r_t$)。収縮の単調判定 $\mu=L_U^2/(R\Delta r)$=6.43 ≤ 20 で余裕あり (無次元なので寸法変更の影響なし)。
...
| $A/A^*(4.19)$ | **14.4430** |
| $r_t$ = 0.800/√(A/A\*) | **0.210505 m** (スロート φ421.01 mm) ← `spec.r_throat` |
| 出口 $T$ / $P$ | 253.0 K / **5235 Pa** ← `spec.p_ambient` |
| $\nu(M_d)$ / $\mu_d$ | 72.06° / 13.81° |
| $r_F/r_t$ / 終端特性線の戻り $r_F/\tan\mu_d$ | 3.8004 / 15.5 $r_t$ (=3.256 m) |
...
- 全長の目安: 全長 ≈ ($x_F$ + $L_U$ + 0.5) $r_t$。$L_c$=8 で **6.40 m**、$L_c$=10 で 6.82 m。
- 参考: スロート質量流量 ≈ **194 kg/s** ($\rho^*$ 2.362 kg/m³, $a^*$ 588.5 m/s, スロート面積 0.1392 m²)。
- **無次元の設計 (R, $L_U$, $L_c$, 壁形状) は寸法によらない** — 出口径変更は `spec.r_throat` の差し替えだけで済む
  (2026-08-18 の 800→1600 mm 訂正も `r_throat` 0.105252→0.210505 のみ。Re は 2 倍になるので粘性 δ\* 段だけ再評価が要る)。
...
(R2/L_U6/L_c8 の実測: θ_max 20.55°、κ₀R 1.000、$x_F$ 23.91 $r_t$ = 全長 6.40 m、AR 10.5 / skew 0.43 PASS)。
