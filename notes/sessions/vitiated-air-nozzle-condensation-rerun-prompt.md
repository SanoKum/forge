...
> `case/44.vitiated_air_wt` の「新条件 (va2/va2fx)」と**同じ手順で**、入口条件だけ次に変えて計算してください:
...
> {dry, 非平衡凝縮 (Kw+HK), 平衡凝縮} = 18 run。各形状の壁点列 (Euler 版) と、全 run の軸上物理量 CSV (h5 の VALUE 全部 + Tsat 後処理列) を出し、
...
4. **投入**: `case/44.vitiated_air_wt/run_va2fx_batch.py` を写して run 番号を README の一覧の続きから振り、
...
5. **後処理**: `python3 axis_csv_va.py run_00NN_*/` (軸 CSV: h5 全 VALUE + `pH2O_post/Tsat_post/subcool_post/S_post`)、
   ...
   `summarize_va2.py` を写して図 (軸 T/Tsat, S, g) と表、`export_wall_va.py --problem problem_<tag>_M<Md>_dry.yaml --kind euler --tag <tag>_M<Md>` (点列)。
...
- **forge binary は 2026-08-18 の修正 (SLAU 二相面温度、平衡凝縮、condTsat) を含むものを使う** (`solver_density_cuda/build/forge`、
  ソースが変わっていたら `ninja -C solver_density_cuda/build`; solverConfig.hpp を触ったら clean rebuild)。
  それ以前の binary は凝縮帯で全エンタルピーが +0.6 % 非保存 (T +4–5 K)。
