...
> **【実験完了 2026-07-20 後続セッション】判別結果: 追いかけっこ (界面抑制) で確定 — ただし発散せず 1 ノード上で飽和。**
> case/38 `run_0016_ret550_nutfloor` (40k) + `run_0017_ret550_nutfloor_cont` (40k, 静定確認) で実施。
> - 床は意図どおり実効 (node2 νt/μ 0.56→33.3 = ρκd·u_τ 厳密一致)。log 層 (y⁺46-138) は **+12.8% → −1.7%** と初の基準内。
> - しかし y_to は 2.11Δy (baseline, 同一定義) → **3.63Δy へ逃走して飽和** (窓推移 3.43→3.53→3.57→3.63→頭打ち)。
>   欠損帯は床上端の node3 へ 1 ノード移動 (解像 55%+モデル 18%)、u⁺ ジャンプ Δu⁺≈2.6 が node3-4 間に再出現、
>   外層 +14% 平行オフセット (baseline +24%)。u_τ=3.85±0.03 静定・実効 Reτ=550。
> - 結論: 床は自身の直上の解像応力立ち上がりを抑制し欠損帯を 1 ノード押し上げる (指紋どおりの界面抑制)。
>   誤差半減の実利はあるが「閉じる」には界面強制系 (解像応力の生成補助) が必要 → ユーザーと設計相談待ち。
> - パッチは **未 commit** (env `FORGE_NUTFLOOR_DMAX`/`FORGE_NUTFLOOR_UTAU` ゲート・未設定で完全不活性)。
>   コピー: `case/38.channel_wmles/run_0016_ret550_nutfloor/nutfloor_experiment.patch`。詳細は case/38 README run 表。
