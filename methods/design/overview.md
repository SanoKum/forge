...
**生産設定 = `wall_mode: cplus` / `n_axis_inv: 2000` / `n_start: 121`** (`dx_wall` は
cplus 経路では未使用 — 壁点の間隔は C⁺ 線の本数で決まる)。旧生産比で
**‖ΔM‖∞ −37% かつ設計 12 倍速**。高速反復用は n=500 (0.240%、**80 倍速**)。
$\varepsilon_M$ rms だけ旧生産が良く (0.020% vs 0.034%)、源項修正後も残る未解明の残件。
