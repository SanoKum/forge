...
は `plans/`、調査メモは `notes/` に置く。個別の検証手順・計算ワークフロー・開発環境は
`procedures/` 配下のハンドブックを参照すること (役割分担は後述)。
...
| 場所 | 役割 |
| --- | --- |
| `methods/` (本ディレクトリ) | 現在の仕様と解説 (恒常的) |
| `plans/` | 変更単位の設計判断 (`active/` 進行中 / `accepted/` 現役 / `archived/` 終了) |
| `notes/` | 調査メモ・作業ログ (`investigations/` / `sessions/`) |
| [`procedures/`](../procedures/README.md) | 運用・開発の手順/ルール (計算手順・発散対処・設定リファレンス・SU2比較・開発環境・コーディング規約・検証ケース) |
...
`methods/` は**現在の仕様**を蓄積する場所で、[`plans/`](../plans/README.md) は
それを変えるための**設計判断 (変更単位)** を置く場所である。
新規機能や設計変更を行うときは、先に該当する `methods/<area>/` の現在仕様を更新し、
その上で [`plans/_template.md`](../plans/_template.md) を雛型に `plans/active/` へ計画を
作って実装に着手する。完了したら計画を `plans/accepted/` (または `archived/`) へ移す。
