...
→ **平板では代表点が厳密に法線上にある**ので、**case/26 の欠損は選択の幾何では説明できない**。
**曲面ベルでは代表点が法線から 30% ずれ**、壁関数が使う $y$ は真の壁距離より 4.6% 小さい。
...
**$u_\tau$ 欠損 (2.3429→2.5716 = −8.9%) の分解**:

| 要因 | 寄与 |
| --- | --- |
| **代表点選択の幾何** | **0%** (法線上に厳密) |
| **粗メッシュ速度場** ($U_t$ が 6.4% 低い) | **+5.28%** |
| **壁法則自体** (基準速度を入れても真値に届かない) | **+4.25%** |

積 $1.0528\times1.0425=1.0976=2.5716/2.3429$ で**厳密に閉じる**。

**壁法則自体のずれの正体**: 壁解像プロファイルを $y^+\approx29$–30 で Reichardt 則と比べると
...
→ **この境界層は $y^+$≈30 で Reichardt 普遍則を 5.1% 下回る** ($x$ によらずほぼ一定)。
**壁関数の前提そのものが本ケースで 5% ずれている**。自由流 $\mu_t/\mu$=65.9 (NASA 標準の 4 桁大) が
候補だが未検証。
