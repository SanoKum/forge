...
→ **$\omega$ は局所 $P$–$D$ 平衡でほぼ完全に決まっている (96%)**。
交差拡散は無視でき、輸送も 4% しかない。**収支は 0.00% で閉じている**ので診断も健全。
...
**予測: E3 は $\omega$ を Menter ピン値とほぼ同じ水準 (1.27e5 vs ピン 1.14e5) へ下げる。**
したがって **E3 単独の効果は 2×2 の $Y_{10}$ (pin=1, bypass=0) = 0.8594 に近く、
目標帯 0.94±0.02 には届かない見込み**である。

これは E3 を否定するものではない (E3 は Dirichlet で殴らずに同じ $\omega$ 水準を
**源項の整合**で得るので、case/40 の過大化を避けられる可能性がある) が、
**E3 だけで目標に到達すると期待してはいけない**ことを実装前に明示しておく。
残り (0.86→0.94) は $\omega$ 以外の機構、あるいは $u_\tau$ を決める代表点速度側にある。
