...
| `run_0063_B_nasa_wf` | 候補 B: NASA 条件 ($k$=8.4215e-5, $\omega$=616.2 → $\mu_t/\mu$=0.009) × y+27 壁関数 | $C_f$/KS=**0.764** (現行 65.9 と同一) → 自由流乱流は無関係 | active (候補 B 棄却の正) |
| `run_0064_B_nasa_ref` | 同 NASA 条件 × y+0.7 壁解像 | $C_f$/KS=0.943、実測/Reichardt=**0.9494** (現行 0.9492 と同一) | active (同上) |
| `run_0065_B_mid_wf` | 中間条件 ($\mu_t/\mu$=1.0) × y+27 壁関数 (単調性確認) | $C_f$/KS=0.764 | active (単調性確認) |
| `run_0066_B_mid_ref` | 同中間条件 × y+0.7 壁解像 | $C_f$/KS=0.943、実測/Reichardt=0.9494 | active (単調性確認) |
...
2. **相対応答が両ケースでほぼ同じ**: case/26 は $0.8445/0.7615-1$=**+10.9%**、
   case/40 は **+12.3%**。→ **ケース固有係数の調整ではなく、両ケースに約 11–12% の
   同方向応答を与える構造的介入**だった、とまでは言える
   (「一貫した物理変更」と断ずるには外部基準との比較が要る)。
   基準に対する絶対誤差で見ると **case/26 は −23.9%→−15.6%、case/40 は −5.5%→+6.1%** で
   **ほぼ対称**。case/40 は「大幅に悪化した」のではなく、**事前ゲート (過大化しない) に
   照らして不合格**というだけである。y+1 基準との**同一定義による直接比較を正式ツール化**
   してから最終評価するのが安全。
3. → **今回試した 3 つの $\omega$ 介入 (Dirichlet ピン / limiter 迂回 / E3) では
   両ケースのゲートを同時に満たせなかった**。case/26 が不足 (−23.9%→−15.6%) で
   case/40 が不足→過剰 (−5.5%→+6.1%) という**符号の違い**が次の手がかりであり、
   **独立性の高い代表点経路を次に診断する**。
...
| **NASA 条件** | **8.4215e-5** | **616.2** | **0.009** | **$k,\omega$ とも内** |
...
| `run_0063` NASA wf | 0.009 | 0.008921 / 0.008799 / 0.008638 | **0.764** | 4417 | — |
| `run_0050` 現行 壁解像 | 65.9 | 65.58 / 65.09 / 64.39 | **0.944** | 5132 | **0.9492** |
| `run_0066` 中間 壁解像 | 1.0 | — | **0.943** | 5103 | **0.9494** |
| `run_0064` NASA 壁解像 | 0.009 | 0.008922 / 0.008799 / 0.008638 | **0.943** | 5110 | **0.9494** |

**BC 変更は確実に場へ効いている** (実測主流 $\mu_t/\mu$ が 4 桁にわたって指定どおり、
流下減衰も 2% 程度)。にもかかわらず:

- **壁関数の $C_f/C_{f,\mathrm{KS}}$ は 0.764 で完全に不変**
- **壁解像も 0.944→0.943 でほぼ不変**
- **Reichardt との差も 0.9492→0.9494 で不変**

### 結論

**自由流乱流は原因ではない。** $\mu_t/\mu$ を **65.9 → 0.009 と 4 桁**下げても、
壁解像プロファイルの Reichardt 相関からの 5.1% 差も、node 壁関数の欠損も**一切動かない**。

→ **5.1% 差の原因候補から自由流乱流を除外する**。残る候補は
壁解像解自身の離散誤差・有限 $Re$・入口/前縁の扱い・物性定義・Reichardt 相関そのものの
適合限界 (この $Re_\theta$ 帯・この BL 状態での)。

**副産物**: 本ケースの現行 $k_\infty$ が NASA 推奨上限の **3206 倍**だと判明した。
物理的な影響は無い (上記) が、**NASA ケースとの比較を「同一条件」と称する場合は
