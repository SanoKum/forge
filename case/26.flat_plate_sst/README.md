...
- **定義**: `res_wall_*.h5` の `twall_x/y/z` (壁半割面 traction) の**接線成分**
  ...
  **壁解像 run では解像値 = 真の壁応力、壁関数 run ではモデル値へ再スケール済み** =
...
| run | $\tau_w$ [Pa] | **/y+1 基準** |
| --- | --- | --- |
| `run_0052_node_yp1_dofonly_regr` (y+1 基準) | 1438.03 | **1.0000** |
| `run_0066` 生産 mode3 | 1368.67 | **0.9518** |
| `run_0071` **E3** | 1635.97 | **1.1376** |
| `run_0072_repdiag` (= run_0066 相当) | 1368.64 | 0.9517 |

**★ E3 の過大化を訂正**: 従来「≈1.061」と書いたのは $\tau_w$ 比 1.1226 を 0.945 に掛けた
**推定値**だった。同一定義で評価し直すと **1.1376** で、**推定より大きく外れている**。
生産構成の 0.9518 は従来の 0.945 とよく一致するので、**ずれていたのは E3 の推定側**。

**代表点法線化のカウンターファクトを接続**:

| | 値 |
| --- | --- |
| `run_0072` の $\tau_w$ (本ツール) | 1368.64 Pa |
| `rep_normal_interp` の「現行」平均 | 1378.43 Pa (**差 0.72%** = 2 経路のクロスチェック) |
| 反実 $\tau_w$ = 1368.64 × 0.6100 | 834.87 Pa |
| **→ y+1 基準比** | **0.5806** (現行 0.9517 から) |

→ **代表点を法線上へ直すと、case/40 は凍結場で 0.952 → 0.58 に悪化する。**
両側が同じ範囲・同じ重み・同じ $\tau_w$ 定義を通っており、カウンターファクトは要素ベースで
$k$ 非依存・本体 `utau` を 1.5e-7 で再構成できることも検証済み。
