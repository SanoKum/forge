...
→ **Reichardt は内部基準より $u_\tau$ を 3.8% 低く出す**が、**Spalding は 2.0% 高く出す**。
**どちらも内部基準には一致しない** (Spalding が正しいわけではない)。

**B. 壁関数場の凍結逆解き** (`run_0063_B_nasa_wf`、現行 Reichardt 比。
本体 `utau` の再構成検証 median **4.2e-8**):
...
**★ A と B の法則間比がほぼ一致する**: Spalding/Reichardt は
壁解像場で **1.0608**、壁関数場で **1.0599** (**0.08% 差**)。
**法則間比はこの $y^+$ 帯で場にほとんど依らない**ので、凍結場の見積りは頑健。

#### 何が言えるか

現行ギャップは $C_f$/KS 0.764 (壁関数) vs 0.943 (壁解像) = $\tau_w$ で **1.234 倍**。
凍結場で Spalding にすると $\tau_w$ ×1.1234 → **0.764 → 約 0.86 相当**で、
**ギャップの約半分**しか埋まらない (残りは代表点速度側の 5.28% など別要因)。
