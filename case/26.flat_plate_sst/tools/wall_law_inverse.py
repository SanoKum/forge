...
def columns(x, y, xmin, xmax):
    """同一 x の壁法線カラムに束ねる (平板・構造格子前提)。"""
    xr = np.round(x, 7)
    out = []
    for xc in np.unique(xr):
        ...
        if len(i) < 5:
            continue
        out.append((xc, i[np.argsort(y[i])]))
    return out
...
def main():
    ...
    mu = h["VALUE/vis_lam"][:]
    x, y = C[:, 0], C[:, 1]
    cols = columns(x, y, a.xmin, a.xmax)
    if not cols:
        raise SystemExit("カラムが作れない (xmin/xmax を確認)")
    ...
    if a.mode == "resolved":
        ...
        rec, r0 = np.array(rec), np.array(ref)
        g = r0 > 0
        rel = np.abs(rec[g] - r0[g]) / r0[g]
        print(f"# ★ 再構成検証: 本体 utau との相対差 median={np.median(rel):.3e} max={rel.max():.3e}")
        if rel.max() > 0.05:
    ...
    print(f'{"壁法則":12s} {"u_tau/基準":>12s} {"tau_w/基準":>12s}   ({base} 比、カラム中央値)')
    for L in laws:
        r = np.array(rows[L]) / ref
        ru = float(np.median(r))
        print(f"{L:12s} {ru:12.4f} {ru**2:12.4f}   (min {r.min():.4f} / max {r.max():.4f})")
    print()
