...
| `points_m5_best_ns.csv` / `nozzle_m5_best_ns.geo` | **物理壁** (`PhysicalNozzleWall` A13 = 非粘性壁 + 上流履歴込み δ\* 相関の法線オフセット)。**実機の壁はこちら** |

物理壁の要点 (Euler 壁との差): δ\* オフセットは入口 0 → スロート **1.15 mm** → 出口 **21.3 mm**。
幾何スロートは 0.81 mm 上流へ移動し半径 81.66 mm (設計 80.505 mm)。出口半径 499.5 → **520.9 mm**
(境界層排除厚を吸収してコア径 1 m を維持)。δ\* は相関ベース (A12 検証: 無帰還 RANS 0.533 %) で、
NS CFD による物理壁の直接検証は未実施。
