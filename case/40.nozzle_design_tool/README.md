...
| `run_0072_repdiag` | **代表点幾何診断** (`FORGE_WF_REP_DIAG=1`、`run_0066` 場から 100 step) | `best_cos`=0.956・**接線オフセット/y=0.306**・`wall_dist[irep]/y`=1.046。凍結場カウンターファクトで**法線化すると面平均 $\tau_w$ が 0.679 倍**と判明 (候補 A は単独では逆効果) — [代表点 plan](../../plans/active/turbulence-node-wf-representative-point.md) | active (§3 の正) |
| `run_0071_E3` | **E3 A/B ($\omega$ 生産を壁法則整合 $S_{\rm wf}^2$ へ)**: `FORGE_WF_OMEGA_SOURCE=1`、`run_0066` 場から 12000 step、他は生産構成と同一 | NaN なし・`check_quasisteady --quantity wall_tau` **STEADY**。**y+1 基準比 = 1.1376 に過大化** (単一評価関数 `wall_tau_eval.py`。従来の「≈1.061」は $\tau_w$ 比を 0.945 に掛けた推定で**過小だった**) → **採用不可** (ただし $\omega$ ピンの 1.237 より遥かに軽い)。壁温 1414.0→1421.1 K。case/26 側は 0.7615→0.8445 で改善 — [plan §5](../../plans/active/turbulence-node-wf-omega-source.md) | active (**E3 不採用の根拠**) |
