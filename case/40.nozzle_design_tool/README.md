...
| `run_0071_E3` | **E3 A/B ($\omega$ 生産を壁法則整合 $S_{\rm wf}^2$ へ)**: `FORGE_WF_OMEGA_SOURCE=1`、`run_0066` 場から 12000 step、他は生産構成と同一 | NaN なし・`check_quasisteady --quantity wall_tau` **STEADY**。**bell $\tau_w$ 1224.2→1374.3 Pa (×1.1226) = y+1 基準比 ≈1.061 に過大化** → **採用不可** (ただし $\omega$ ピンの 1.237 より遥かに軽い)。壁温 1414.0→1421.1 K。case/26 側は 0.7615→0.8445 で改善 — [plan §5](../../plans/active/turbulence-node-wf-omega-source.md) | active (**E3 不採用の根拠**) |
