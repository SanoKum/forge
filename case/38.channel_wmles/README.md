...
| `run_0005_ret550_long` | **ベースライン長時間発展**: res_40000 から設定変更なしで 100k step (~250 FT)。u_τ プラトー到達後にのみ評価 (ユーザー指示) | **u_τ 静定を確認** (step 30k 以降 150 FT で 3.72±0.08, トレンド +1.3%/窓 = OSCILLATING)。**静定後評価: 不合格** — ①大域運動量 ⟨ρu_τ²⟩=10.0 Pa vs fδ=17.4 Pa (−43%)、②log 層 u⁺ +42% (基準 \|5\|%)、③第 1–2 内点間に Δu⁺≈15 の非物理ジャンプ。**構図: I1-I2 面が乱流応力を運べず、分子粘性レベルの巨大勾配で応力を運ぶ歪な擬似平衡** (コアと壁が脱結合、u_b 102 へ緩慢加速)。壁モデル自体はマッチング点に忠実 (健全) | ref (**ベースライン確定判定**) |
| `run_0006_ret550_lim0_s002` | **較正 A/B ①**: res_100000 から limiter -1 (off) + σ0.02 に変更し 40k step (venkata が乱流極値を刈り σ0.05 と合わせ近壁ストリークを殺す疑い) | (実行後に記入) | active |
