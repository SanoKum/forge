...
usage: python3 axis_csv_d155.py <run_dir> [...]  → <run_dir>/axis_values.csv (列: x_m, x_over_rt, r_m, 全 VALUE 名 [ベクトル/3成分はそのまま])"""
import sys, json
...
for rd in map(Path, sys.argv[1:]):
    ...
    # p_H2O = Y_H2O P (Y1 があれば TP split の H2O 質量分率 → モル分率換算; 無ければ設計組成 x_H2O=0.13560 (イソブタン φ=0.9))
    def psat_l(T):
