...
| `run_0026_regr_oldbin` / `run_0026_regr_newbin` / `run_0027_regr_newbin_rerun` | 2026-09-05 の出口 γ 修正 (plan outlet §2.12) の A/B: run_0025 収束場から 300 step を修正前/修正後バイナリで再実行 + 修正後の再現性 | 修正前後で残差 max 0.57 % 差 (再現ノイズ 1.5e-5) = 壁近傍の亜音速出口列が正しく変わった挙動変更。出口プロファイルの再確認は `run_0028` で完了 | 破棄予定 (回帰記録) |
| `run_0028_exit_recheck` | **3 バグ修正後 (876ec8b4) の出口プロファイル再確認**: run_0025 `res_20000` から `restart_conserved.py` で同一メッシュ restart、修正後バイナリで 5000 step 継続 (設定は run_0026 と同一) | **Phase 3 ベストの出口結論は維持**: X_H₂O ピーク 0.504 @ 1.96 cm (修正前 0.505 @ 1.96)、全温ピーク 1.08 @ 1.85 cm・OH max 3.14e-2 は不変 (`exit_react_5000.png`)。出口指標の 5000 step ドリフト ≤0.1 %。γ 修正+継続の差は壁直近の亜音速出口列に局在 (出口線 P 最大 13.5 % @ y=0, M 0.77; プロファイル比較域は不変)。`check_convergence`: NOT CONVERGED (rms_ro 7.8e-6 falling, SST プラトー = run_0025 と同質)。`check_quasisteady --quantity machmax,pmax`: **STEADY**。着火位置 (Y_OH>1e-4) は 4.36→3.93 cm と緩慢な上流ドリフト継続 (既知の早着火・未解決、この量は DRIFTING) | ref (**出口再確認**) |
