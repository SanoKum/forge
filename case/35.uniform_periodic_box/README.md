...
| `run_0052_node_h2_ignition_strang` | run_0049 + **Strang 分離** (`chemistry.strang: 1`, dt 5e-9, 10000 step) | 着火遅れ 32.0 µs (Cantera 32.2)、平衡 T **2945.1 K** (Cantera 2943.9; ソース項経路 run_0049 の 2948.4 より近い)、場は厳密一様 (\|u\|=0)。`check_convergence` の rising 指摘は残差が機械ゼロ級 (1e-12) で無意味。**913 s** (run_0049 の 94 s の 10 倍: sub-cycle カーネルが重い、要最適化) | ref (Strang 検証) |
| `run_0053_node_h2_ignition_strang_dt10x` | 同 Strang, **dt 5e-8 (10 倍)**, 1000 step | 着火遅れ 32.0 µs、平衡 2945.2 K で dt 5e-9 と同一 = 化学時間 (~1e-7 s) と同程度の dt でも Strang + BE sub-cycle で精度維持。155 s | ref (Strang dt 独立性) |
| `run_0054_strang_v2_dt5e9` / `run_0055_strang_v2_dt5e8` | Strang **v2** (刻みを主要種相対変化 <20 %・\|ΔT\|<30 K で受理、非反応セルは 1 sub-step) の再検証, `.build-speed` バイナリ | 着火遅れ 32.0 µs・平衡 2945.1 K で v1 と同一 (Cantera 32.2 / 2943.9)。所要 605 s (v1 913 s; GPU 共有中の計測で参考値。729 ノードは latency 律速) | ref (Strang v2) |
