#!/usr/bin/env bash
# 構造メッシュ・[1.8,1.85] の細かい背圧 sweep: Ps 1.81/1.82/1.83/1.84。
# GPU 競合回避のため up-sweep 完了 & GPU 空きを待ってから開始。run_0034(Ps1.8) から chain。
set -uo pipefail
ROOT=/home/sano/work/forge
CD=$ROOT/case/36.passive_pseudoshock_control
export LD_LIBRARY_PATH=/usr/lib/x86_64-linux-gnu/hdf5/serial:${LD_LIBRARY_PATH:-}
cd "$CD"

# up-sweep の完了 & forge プロセスが居なくなるまで待機
until grep -q "UP-SWEEP DONE" up_sweep_struct.out 2>/dev/null; do sleep 60; done
until [ -z "$(pgrep -x forge)" ]; do sleep 30; done
echo "[fine_sweep] up-sweep done, GPU free -> start fine sweep"

mkbcond(){ # $1=file $2=Ps[Pa]
cat > "$1" <<EOF
inlet:
  physID: 1
  kind: inlet_uniformVelocity
  outputHDFflg: 0
  ints:
  floats:
    Ux: 458.6
    Uy: 0.0
    Uz: 0.0
    ro: 11.733
    Ps: 617800.0
    k: 31.547
    omega: 3.0018e7
outlet:
  physID: 2
  kind: outlet_statPress
  outputHDFflg: 0
  ints:
  floats:
    Ps: $2
    Pt: $2
    Tt: 288.15
wall_slip:
  physID: 3
  kind: slip
  outputHDFflg: 0
  ints:
  floats:
wall:
  physID: 4
  kind: wall
  outputHDFflg: 0
  ints:
  floats:
EOF
}

prev="run_0034_struct_porous_bp1p8"
tags=(run_0040_struct_porous_bp1p81 run_0041_struct_porous_bp1p82 run_0042_struct_porous_bp1p83 run_0043_struct_porous_bp1p84)
pss=(1810000.0 1820000.0 1830000.0 1840000.0)
for i in 0 1 2 3; do
  R=${tags[$i]}; ps=${pss[$i]}
  mkdir -p "$R"
  cp "$prev/solverConfig.yaml" "$prev/probe.yaml" "$prev/passive_porous.h5" "$R/"
  python3 restart_field.py "$prev/res_80000.h5" "$R/passive_porous.h5" >/dev/null
  mkbcond "$R/bcondConfig.yaml" "$ps"
  echo "=== fine-sweep $R  Ps=$ps  (continue from $prev) ==="
  bash "$ROOT/solver_density_cuda/tools/run_case.sh" "$CD/$R"
  if ls "$R"/res_nan_*.h5 >/dev/null 2>&1; then echo "DIVERGED at $R — stop"; break; fi
  prev="$R"
done
echo "FINE-SWEEP DONE"
