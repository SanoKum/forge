...
| `run_0077_react_li_dualtime` | run_0073 `res_30000` から **dual-time** (`unsteady 1, dualTime 1`, dt 1e-5 s 固定 = ジェット部 CFL 6.5, nStepInner 5, cfl_pseudo 0.5, precond 2, 外周 slip), Li, 10000 step = 物理 0.1 s (coflow 通過時間 71 ms の 1.4 倍), 出力 10 ms 毎 (切り分け順序 ④: 擬似時間リミットサイクルが物理時間発展でどうなるか) | **物理時間では減衰・定常化**: 出口質量流束 0.032 (0 ms) → 0.041 (30 ms) → 0.034 (60 ms) → **0.037–0.038 (80–100 ms, 理論 0.037)**、z/d 26 の 10 ms あたり ΔT 135 → 45 → 7 → … → ≤2 K、逆流なし、`ignx` 0・T_max 1570 K 不変。`check_quasisteady` (末尾 40 %) は 40–60 ms の減衰揺り戻しを含み DRIFTING だが末尾 30 ms は収束。**擬似時間の周期 15k step 脈動は定常反復固有と確定**。燃焼場は定常 run_0073 と同一 (中心軸 T 差 ≤15 K, z/d 26 半径 T 差 ≤20 K) — 実験との差 (z/d 9–14 で +150–220 K, z/d 20 で +200 K) はそのまま = モデル (TCI) の問題。図 `compare_cabra_7000.png` | ref (**P0-2 の結論**) |
