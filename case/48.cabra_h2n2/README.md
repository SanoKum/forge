...
| `run_0078_react_li_extdomain2` | 同じ領域拡張 (Lx 0.40, R 0.20) だが**近傍解像度を旧メッシュに一致** (`--nx 354 --ny_co2 82`: 出口の軸方向初期幅 0.20 mm・r=30 mm 外側の初期幅を旧値に合わせ、外側だけ粗く延長), run_0073 から補間, cfl 1.0, Li, 30000 step | (投入待ち: GPU は run_0077) | 準備済 |
| `run_0077_react_li_dualtime` | run_0073 `res_30000` から **dual-time** (`unsteady 1, dualTime 1`, dt 1e-5 s 固定 = ジェット部 CFL 6.5, nStepInner 5, cfl_pseudo 0.5, precond 2, 外周 slip), Li, 10000 step = 物理 0.1 s (coflow 通過時間 71 ms の 1.4 倍), 出力 10 ms 毎 (切り分け順序 ④: 擬似時間リミットサイクルが物理時間発展でどうなるか) | (実行中) | active |
