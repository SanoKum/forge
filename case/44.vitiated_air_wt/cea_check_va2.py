"""NASA CEA2 (凍結流, 5 成分固定) と forge node Euler TP 軸解の比較 — va2 新条件 (Pt 11.37 bar, Tt 1058 K, M4.19 dry run_0049).
...
RUN = CASE / (sys.argv[1] if len(sys.argv) > 1 else "run_0049_va2lp_M4.19_dry")
PT, TT = 1.137e6, 1058.0     # va2 新条件
S = 0.21056  # r_t [m] (M4.19 va2)
CEA_OUT, CEA_CASE, SUPAR = "va2_cea.out", "v2_fz", 14.436
...
print(f"CEA frozen exit (Ae/At=14.436): M={cea_exit['M']:.3f} P={cea_exit['P']*1e5:.0f} Pa T={cea_exit['T']:.2f} K rho={cea_exit['RHO']:.5f} u={cea_exit['u']:.1f} a={cea_exit['a']:.1f}")
