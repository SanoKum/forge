...
読み方:
- **非平衡 (実際に起きること)**: 閾値は前節と同じ **M_d≈4.7 (T≈210 K)**。4.19–4.4 は g=0 (過冷却 9–25 K のまま凍結)、4.5 は微量 (1 %)、
  4.75/5.0 は x≈8–9 r_t (Wilson 点 T 205–210 K, S≈100–300) で一気に凝縮し 43–54 % → 出口 T +35–45 K、M −2〜−4 %、ε_M 8–11 %。
- **平衡 (熱力学的上限)**: 飽和線到達 x≈5.7–6.6 r_t (T≈260 K) から連続的に凝縮。出口で H₂O の 9 % (M4.19) → 45 % (M5.0)、
  出口 T は 243–259 K に張り付き (飽和線)、**出口 M は 4.13 / 4.17 / 4.21 / 4.24 / 4.33 / 4.42** (設計 4.19–5.0)、‖ΔM‖∞ 1.5–12 %。
  M_d 4.19–4.4 では非平衡なら無害だが**平衡なら 9–24 % 凝縮して M が 1.5–4.4 % 落ちる** — 実際は核生成障壁 (S≲10) で起きない (非平衡が正)。
