"""NASA CEA2 (凍結流, 5 成分固定) と forge node Euler TP 軸解の比較 (case/44 M4.19 vitiated air).
実行: python3 case/44.vitiated_air_wt/cea_check_va.py [run_dir]
前提: cea/va_cea.out (NASA CEA2 の出力; .venv-cea/nasa_cea/FCEA2 で `echo va_cea | FCEA2`)."""
import re, sys, json
...
RUN = CASE / (sys.argv[1] if len(sys.argv) > 1 else "run_0019_va_R2_LU6_Lc8_newbin")
PT, TT = 1.14e6, 1060.0
S = 0.210505  # r_t [m]
...
cea = parse_cea(CASE / "cea" / "va_cea.out", "va_fz")
# 出口 supar=14.443 の行 (Pinf/P=217.77 の重複) を分離
i_sup = np.where(np.isfinite(cea["AeAt"]) & (np.abs(cea["AeAt"] - 14.443) < 1e-3))[0]
...
print(f"CEA frozen exit (Ae/At=14.443): M={cea_exit['M']:.3f} P={cea_exit['P']*1e5:.0f} Pa T={cea_exit['T']:.2f} K rho={cea_exit['RHO']:.5f} u={cea_exit['u']:.1f} a={cea_exit['a']:.1f}")
...
Rf = P / (ro * T); R_cea = 8314.462618 / 29.081
print(f"forge R=P/(rho T): {Rf.mean():.3f} ± {Rf.std():.3f}  (CEA MW 29.081 → R={R_cea:.3f}; design R=285.911)")
...
def mdot_at(i):
    r = Y[i]; return np.trapz(ro2[i] * Ux2[i] * 2 * np.pi * r, r)
A_star = np.pi * S**2
...
def mavg(q): return np.trapz(q * w, r) / np.trapz(w, r)
...
print(f"  CEA frozen 1D @A/A*=14.443: M={cea_exit['M']:.3f} T={cea_exit['T']:.2f} P={cea_exit['P']*1e5:.0f} u={cea_exit['u']:.1f}")
...
fig.suptitle(f"case/44 vitiated air Pt=11.4 bar Tt=1060 K: forge node Euler TP axis vs NASA CEA2 frozen expansion ({RUN.name}, {res.name})", fontsize=11)
