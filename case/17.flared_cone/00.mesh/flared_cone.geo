...
cone_lines[1] = newl;
Circle(cone_lines[1]) = {pList1[1], pFlareCent, pList1[2]}; Transfinite Line {cone_lines[1]} = nx_1 Using Progression 1.003;
//cone_lines[2] = newl;
...
r_flare_inlet = r_flare/1.3;

pInletCent = newp;
Point(pInletCent) = {0.0, 0.0, 0.0, lc};

pFlareInletCent        = newp;
Point(pFlareInletCent) = {0.0, r_flare_inlet+b, 0.0, lc};
...
pList_in1[1] = newp;
Point(pList_in1[1]) = {x_le_in, b, 0.0, lc};

x2 = L-0.0;
y2 = r_flare_inlet+b - Sqrt((r_flare_inlet)^2 - x2^2);

pList_in1[2] = newp;
Point(pList_in1[2]) = {x2, y2, 0.0, lc};
...
//// inlet lines
inlet_lines[0] = newl;
Ellipse(inlet_lines[0]) = {pList_in1[0], pInletCent     , pList_in1[1], pList_in1[1]}; Transfinite Line {inlet_lines[0]} = nx_0;
inlet_lines[1] = newl;
Circle (inlet_lines[1]) = {pList_in1[1], pFlareInletCent, pList_in1[2]}; Transfinite Line {inlet_lines[1]} = nx_1 Using Progression 1.003;
//inlet_lines[1] = newl;
...
span_lines[2] = newl;
Line(span_lines[2]) = {pList1[2], pList_in1[2]}; Transfinite Line {span_lines[2]} = ny Using Progression 1.00;
//span_lines[3] = newl;
