...
cone_lines[1] = newl;
Circle(cone_lines[1]) = {pList1[1], pFlareCent, pList1[2]}; Transfinite Line {cone_lines[1]} = nx_1 Using Progression 1.001;
cone_lines[2] = newl;
...
pList_in1[1] = newp;
Point(pList_in1[1]) = {x1-0.05 , y1*5.2 , 0.0 , lc};
...
// *** inlet line 1 ***
pList_inner[0] = newp;
Point(pList_inner[0]) = {-0.45, 0.42, 0.0, 1.0};
...
INLET_LINES[1] = newl;
Bezier(newl) = {pList_in1[1], pList_inner[1], pList_inner[2], pList_in1[2]}; Transfinite Line {INLET_LINES[1]} = nx_1 Using Progression 1.0005;
...
span_lines[1] = newl;
Line(span_lines[1]) = {pList1[1], pList_in1[1]}; Transfinite Line {span_lines[1]} = ny Using Progression 1.00;
span_lines[2] = newl;
Line(span_lines[2]) = {pList1[2], pList_in1[2]}; Transfinite Line {span_lines[2]} = ny Using Progression 1.015;
span_lines[3] = newl;
...
//temp Physical Surface("wall", 3) = {144, 122, 100, 78, 56, 34};
//temp Physical Surface("axis", 4) = {46};
//temp Physical Surface("side1", 5) = {25, 24, 23, 22, 21, 20};
//temp Physical Surface("side2",6) = {69, 91, 113, 135, 157, 47};
//temp Physical Volume("fluid", 7) = {1, 2, 3, 4, 5, 6};
//temp //+
//+
Physical Surface("inlet", 1) = {38, 60};
//+
Physical Surface("outlet", 2) = {56};
//+
Physical Surface("wall", 3) = {30, 52};
//+
Physical Surface("axis", 4) = {42};
//+
Physical Surface("side1", 5) = {21, 20};
Physical Surface("side2", 6) = {43, 65};
