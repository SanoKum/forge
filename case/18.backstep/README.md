...
| `run_0036_node_rans_2d` | **node-centered (median-dual) 検証**: 新規平面メッシュ `mesh/backstep_planar.geo` (10997 nodes, `gmsh -2`, push 無し)、`discretization:node`+`gradLSQ:1`+`nodeWallViscGradFlux:1`、cell 収束場から最近傍 cross-mesh restart、RANS SST RK3 cfl0.3 | **DIVERGED @step18**。roOmega が step18 で 9.4e25→NaN。`residual_history.png` | 削除済み(2026-08-31) (記録) |
| `run_0037_node_laminar_2d` | run_0036 を **層流** (LESorRANS:0) に。turbulence 固有か切り分け | **同じく DIVERGED @step18** → turbulence 非依存 | 削除済み(2026-08-31) (記録) |
| `run_0038_node_lam_nolsq` | 層流 + `gradLSQ:0`+`nodeWallViscGradFlux:0` (検証済み M3 経路) | **同じく @step18** → 未検証フラグでも convMethod でもない | 削除済み(2026-08-31) (記録) |
| `run_0039_node_lam_diag` | 発散直前 (16 step, out interval 2) ダンプで空間切り分け | **種=段差直後 再循環域 (x≈2.26,y≈0.56-0.62) の隣接 CV odd-even (チェッカーボード) ro 振動**が 2step ごと倍々成長→step18 爆発。種域 Mach≈0.005-0.03 (淀み) | 削除済み(2026-08-31) (記録) |
| `run_0040_node_lam_dirichlet` | 働く壁レシピ `nodeWallDirichlet=1`+`bndFirstOrder=1`+cfl0.1 を適用 (case/29 由来) | **同じく DIVERGED @step18** → 壁レシピでは直らない (種は壁でなく内部淀み域) | 削除済み(2026-08-31) (記録) |
| `run_0041_node_lam_midfx` | + `nodeMidpointFx=1` (median-dual checkerboard 低減フラグ) | **同じく DIVERGED @step18** | 削除済み(2026-08-31) (記録) |
| `run_0042_node_lam_precond` | + `lowMachPrecond=2` | **step0→step1 即 NaN** (前処理が node で破綻) | 削除済み(2026-08-31) (記録) |
| `run_0043_cell_lam_planar` / `run_0045_cell_rans_planar` | **対照: 同じ planar 2D を cell で** (層流/RANS) | **どちらも DIVERGED** (step26/27、node と同じ再循環域 x≈2.46) → 発散は node 固有でない | 削除済み(2026-08-31) (記録) |
| `run_0047_rerun0002_rk3` / `run_0048_rerun0002_oldbuild` | run_0002 を現 build / 旧 build(06-14) で再計算 | **両方 step28 DIVERGED** (収束場 restart でも) → build リグレッションでなく RK3 explicit が元から不安定 | 削除済み(2026-08-31) (記録) |
...
| `run_0112_les40k_iles` | 長尺 SGS 比較 (σ0.05+c') — **step1052 で中断** (前処理散逸の開発を優先、σ設定も再検討対象に) | VERDICT 記録済。`run_0113/0114` (WALE/σ) は未実行のままディレクトリ破棄 | 削除済み(2026-08-31) (中断記録) |
...
| `run_0128_iddes_regr_mode1` | **IDDES Phase 2 の DDES 回帰**: run_0125 と同一入力 (同 restart md5)・DESmode:1 のまま、Phase 2 実装後バイナリで 500 step ([iddes plan §4.6](../../plans/active/turbulence-iddes-sst.md)) | 全 12 フィールド (保存量+P/T+`fd_shield`/`l_des`/`rd_des`) の vs run_0125 max\|Δ\|/relL2 が **run_0130 のノイズフロアと同水準 → DESmode:1 経路は数値的に不変**。NaN 無し | ref (mode1 回帰) |
| `run_0129_iddes_mode2` | **IDDES 機能確認① (RANS 的場)**: run_0125 と同一入力で `DESmode: 2` のみ差分、500 step implicit | NaN 無し。f̃_d ゾーニング正常 (付着 BL med **1.000**=RANS 遮蔽・`l_des/l_rans`=1.000 厳密、せん断層 med 0.036=LES 分岐、泡内 1.000=RANS)。f_e は付着 BL 近壁のみ活性 (frac 24%, max 0.98)・他域 0。rd clamp 飽和 0.7% | ref (IDDES 機能① implicit) |
| `run_0130_iddes_regr_mode1_head` | run_0128 の**ノイズフロア基準**: 同一入力を HEAD (Phase 2 変更前) バイナリで再実行 (atomicAdd run-to-run 揺らぎの実測) | vs run_0125: 例 roUx max\|Δ\| 4.5e-4, fd 1.1e-2 — run_0128 の差と同値 → 回帰判定の物差し | 削除済み(2026-08-31) (判定完了後) |
| `run_0131_iddes_mode2_dev` | **IDDES 機能確認② (発達乱流場)**: run_0126 res_30000 (解像 3D 乱流飽和済) を restart3d.h5 にパッチし `DESmode: 2` で 3000 step unsteady RK3 cfl1 (WMLES 分岐 f_dt の解像乱流応答を見る) | **合格・NaN 無し** (res_3000 瞬時場)。①**WMLES スイッチ動作**: 遠壁 (f_B<0.01) で f_dt med=**0.992**・frac(f_dt>0.9)=67% = 解像乱流を検知し WMLES 分岐へ、②**壁直近ガード**: f_B>0.999 帯 (37680 cells) で min f̃_d=**1.0000** = RANS 層保持厳密、③せん断層 f̃_d≈0.001 (LES)・`l/l_RANS` 0.537 ≒ DDES 0.524、④f_e は付着 BL/再付着後近壁帯のみ活性 (frac 20-29%, max 0.99)、⑤解像乱流維持 (せん断層 Uz rms 4.97→5.13 m/s、モデル νt 0.076→0.061)。残差プラトーは非定常想定どおり | active (IDDES 機能② unsteady) |
