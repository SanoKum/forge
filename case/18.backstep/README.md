...
| `run_0130_iddes_regr_mode1_head` | run_0128 の**ノイズフロア基準**: 同一入力を HEAD (Phase 2 変更前) バイナリで再実行 (atomicAdd run-to-run 揺らぎの実測) | vs run_0125: 例 roUx max\|Δ\| 4.5e-4, fd 1.1e-2 — run_0128 の差と同値 → 回帰判定の物差し | 破棄予定 (判定完了後) |
