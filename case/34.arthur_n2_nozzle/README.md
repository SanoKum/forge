...
| `run_0006_cond_n2` | 非平衡凝縮 Phase 2 (`condensation:1`, CPG 二相 EOS + 核生成/成長ソース)。run_0003 収束 dry 場から restart、`restart_dry.h5` | **bounded・NaN なし**を確認: g∈[0,1] (max 0.24, mean 0.029)、最小 T が dry 27.8K→34.6K に上昇 (潜熱)。**g=0 域は dry と完全一致** (上流 P_cond/P_dry=1.00)。ただし g が過大 (paper ~0.75-2.3% に対し 22%)=収束 dry からの一斉 onset アーティファクト (S~3e4 一斉核生成)。定量一致は次段 (膨張流からの physical onset / レート較正)。`residual_history.png` | active (Phase 2 骨格・bounded 確認) |
