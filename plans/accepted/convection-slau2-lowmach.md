...
- **やる**:
  ...
  - `methods/convection/{theory,implementation}.md` に SLAU2 を追記。
...
- 理論: [`methods/convection/theory.md`](../../methods/convection/theory.md) の **SLAU 節**
  (§ 質量流束と圧力束, L323 付近)。ここに SLAU2 の圧力束式と低マッハ根拠を追記する。
- 実装: [`methods/convection/implementation.md`](../../methods/convection/implementation.md) に
...
1. **docs 先行更新**: `methods/convection/theory.md` SLAU 節に SLAU2 圧力束式と低マッハ根拠を
   追記 / `methods/convection/implementation.md` に `solver: SLAU2` を追記 / `methods/index.md`
   目次は構成不変なら更新不要 (確認のみ)。
...
- **検証ケース** (`procedures/verification/README.md` 準拠):
...
- `methods/convection/theory.md`, `methods/convection/implementation.md` (+ `methods/index.md` 確認)。
- 既存ケースへの影響: `solver: SLAU` の挙動は不変 (新オプション追加のみ)。
- `case/23.axi_nozzle/mesh_3d_m4/README.md` に SLAU2 の選択肢を追記 (任意)。

## 8. 完了条件

- [x] `methods/convection/theory.md` 更新済み (SLAU2 節 + 低マッハフロアとの関係を正確化)
- [x] `methods/convection/implementation.md` 更新済み (dispatch 表 + `slauVariant` 分岐)
