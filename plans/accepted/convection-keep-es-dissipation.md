...
- `2026-07-19` — **L3 64³ + 実 DNS データで σ 確定** (残 open「64³ 以上での DNS 定量比較」の消化)。
  - DNS 参照を近似帯から実データへ: `case/09.Taylor-Green/ref_dns/TGV_Re1600.dat`
    (**Dairay+2017 JCP, Incompact3d 512³**, 要引用)。真のアンカー **K/K0(10)=0.596・ε* ピーク 0.1029 @ t*=8.98**。
    旧近似帯 (0.50-0.57) は誤りで、それに基づく「32³ σ=0.03 が帯内」判定は撤回
    (実データでは 32³ σ=0.02 の 0.594 がほぼ厳密一致だった)。
  - **64³ (`case/09...run_0035/0036/0037`)**: σ=0/0.02/0.03 とも層流期 (t*=4)・終値 (t*=10) の
    K/K0 誤差 ±3% 内 = 32³ の「層流期と終値の両立不可」は解像度律速と確定。
    σ=0 (+1.7%) と σ=0.02 (**−1.5%**) が DNS を対称に挟む。ピーク時刻は σ=0 で 8.96 (DNS 8.98)。
