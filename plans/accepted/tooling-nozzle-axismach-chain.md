...
- `2026-08-15` — **出口位置の定義を終端特性線に修正** (ユーザ指摘「えいやの見積もりでなく
  MOC で飛ばす特性線から出口位置を定めてほしい」)。調査で判明した実態: (a) えいや見積もり
  $2.3 r_F\sqrt{M_d^2-1}$ は**軸 target の延長量 (場の計算範囲)** にしか使っておらず長さの
  決定要因ではなかった、(b) 実際の切断は**壁角しきい値 0.05°** で、壁角が $F$ へ漸近的に
  落ちるためこれが $F$ の **2.59 $r_t$ 上流**で発火していた、(c) その結果**一様コアが出口面に
  届いていなかった** (コア半径 = 出口半径の 80%)。`moc_inverse.terminal_exit` を実装し
  ($E$ 発 C⁺ を場の中で追跡 → 壁流線との交点)、`exit_mode: characteristic` を既定化。
  検証 `run_0048_axismach_charexit`: $x_F$ 19.708→22.263、着地点で $\theta_w=1.0\times10^{-10}$°・
  $M_w=4.0000000$ (自己整合の検算)、$x_F-x_E$=12.632 が理論 12.680 と 0.4% 一致、
  **コア面 38/65→65/65 (出口全面が一様)**。ΔM ゲートは維持 (0.464% $M_d$)。
  margin 感度: 2.3→3.0 で $x_F$ 変化 0.08% (長さは MOC が決めている)。margin 不足時は
  旧方式が黙って「計算範囲の端」を返していたのに対し新方式は明示エラー。
  解析 = `case/41.wind_tunnel_design/analyze_axismach_exit.py`。
- `2026-08-15` — **1 パスの wall-clock 内訳を実測** (`axismach_timing.json`,
  `time_axismach_chain.py`): 合計 **6.8 分**のうち**逆 MOC が 90.8%** (369 s @ n_axis=2000)、
  CFD は **30 秒 (7.4%)** のみ (soft 3000 step 7.1 s + 本段 12000 step 22.8 s, RTX 3060)。
  メッシュ+IC 2.0 s (0.5%)、評価+アンカー抽出 5.4 s (1.3%、うち $x_{reach}$ 感度一式 4.2 s)。
