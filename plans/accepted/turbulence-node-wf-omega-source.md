...
- [x] **E3 実装 (`wf_irep_flag` + `wf_sprod`) と判定**: case/26 で 0.7615→**0.8445**
      ...
      **case/40 で統一評価 $\tau_w$ が生産比 1.1954 倍・y+1 内部基準比 1.1414 に過大化** →
      **このままでは採用不可** ($\omega$ ピンの 1.2352 より軽い)。
      振れ幅は case/26 +10.9% / case/40 +19.5% で**同程度ではない**
      (旧記載「同程度」「残りは単一ノブで閉じない」は評価系を混ぜた推論なので**撤回**)。
      符号が逆向き (case/26 不足 / case/40 過剰) である点のみが残る事実
...
- `2026-08-13 (3)` — **E3 実装・判定**。`wf_sprod` ($S_{\rm wf}^2$) を `ransWallFunction` で作り
  ...
  **case/40**: 統一評価 (`wall_tau_eval.py`, 範囲 x=[0.0102,0.0685]) で
  生産 1388.35 Pa → E3 1659.59 Pa = **生産比 1.1954 / y+1 内部基準比 1.1414** に過大化。
  (旧評価の 1224.2→1374.3 Pa = **応答比 1.1226** は別評価系の値であり、
  内部基準比 1.1414 と**等号で結んではならない**。さらに旧記載「≈1.061」は
  $\tau_w$ 比を 0.945 に掛けた推定で**撤回済み**。) →
  **採用不可**。ただし $\omega$ ピン (1.237) より遥かに軽く、Dirichlet を使わず
  $k$/$\omega$ の非対称を解消する点は正しい方向。
  統一評価での振れ幅は **case/26 +10.9% / case/40 +19.5%** で**同程度ではない**
  (「同方向に同程度」は評価系を混ぜた旧主張なので**撤回**)。
  確定しているのは **case/26 不足 / case/40 過剰という符号の違い**が残ることのみ。
  次は $u_\tau$ を決める代表点速度側。
- `2026-08-13 (4)` — **レビュー指摘 3 点を修正し、後継 plan を起票**。
  ...
  **【この行の数値・解釈は §9 末尾で撤回済み】** 統一評価では case/26 +10.9% / case/40 +19.5% で
  「約 11–12% の同方向応答」は成立せず、case/40 の絶対誤差も −4.51%→+14.14% で対称ではない。
