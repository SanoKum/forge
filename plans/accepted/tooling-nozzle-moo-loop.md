...
- `2026-08-13` (追補) — **前線平滑化 + Rao 判定の精密化** (ユーザ指摘「前線がガタガタ / 本当に
  Rao が載っているか / Rao 超えの可能性は」への回答): (i) キャンペーン再開で infill +20 評価
  (計 70)、(ii) **固定 L=5.9713 (rao80 と同一長) の (θn,θe) 直接最適化** (`run_0076_fixedL597`,
  サロゲート支援 6 評価)。結果: 最適は **θn24.5–28.5×θe8–11 の平坦な台地** (η=0.96741 @
  θn25.8/θe10, settled) で、**チャート点はその +0.133% 下 = 事前判定帯 ±0.15% 内 → 「Rao は
  前線上」で確定** (Rao 超えではない。チャート θn33.8° は本評価系ではやや過大という一貫した
  小シフト = digitize 誤差+粘性シフトの範囲)。前線の「θe 下限 4° 張り付き」も固定 L では
  最適 θe=10° であり、θe の減少は長 L 側のみ = **Rao チャート自身の θe(ベル長) 減少傾向の
  再現**でアーティファクトではない。確定前線 = 77 評価から 23 点 (新規 7 点もポリッシュ
  済み 7/7 settled)。成果物 = `pareto_merged_final.{json,png}`。切り分けの残し
  (ギャップが開いた場合用に温存): Euler A/B (Rao は非粘性理論なので非粘性ならチャートが
  勝つはず)・推力の検査体積別会計・倍解像順位保存。
...
- `2026-08-13` — §5 ステップ 5 完了 (本番キャンペーン `run_0074_moo_top_r1`): **50 評価 28 分で
  ...
  中庸。(ii) 唯一の FAIL (θn30/θe4/L5) は残差 PASS・ALL STEADY だが ṁ/ṁ_1D=1.034 の非物理解
  — 物理ゲートが機能した実例。(iii) conv 内訳 = PASS 37 / STALLED (プラトー) 12 — **確定版
  前線を出す前に「η ドリフトゲート + チャンク継続」で前線 19 点をポリッシュする**
  (ユーザ指摘 2026-08-13。ステップ 6 Rao 照合の前提として実施予定)。図 = `pareto_r1.png`。
