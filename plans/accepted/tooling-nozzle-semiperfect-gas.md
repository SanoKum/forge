...
- `2026-08-16` — **申し送り解消**: `inlet_Pressure_d` TP 分岐の緩和係数 rf を 0.5→0 (速度参照のみ = CPG 分岐と同じ閉包)
  に修正 ([knot plan](tooling-nozzle-axismach-knot-law.md) 作業中に M6 で発散として顕在化)。M4.2 再実行 (run_0050) で
  残差床 3e-2 → 9e-7、入口 P/Pt 0.9986–0.9993 (全圧超過なし)、‖ΔM‖∞ 0.138% Md・ε_M 0.025%。
  配管長 0.5→2 r_t (run_0029) でも不変 → 入口 BC 自体の性質。設計側の手は尽くした。
- `2026-08-17` — **低温外挿の再検証 (datum 修正後)**: run_0022 の「200 K 未満 cp 凍結でも発散」は datum 未修正下の
  実験で、凍結の可否を判定していなかった (発散は datum のせい)。datum 修正済みで再試験 (run_0030、
  run_0027 と同じ IC から 6000 step): 発散なし、軸 M 差 3e-3・出口 M 4.2011 vs 4.2010 で同一。
  ρ/P の 3% 差は入口 BC の緩和振動 (P/Pt 1.03) の位相差で低温側の効果ではない。**収束場の最低 T は
  247 K で 200 K 未満の点はゼロ** — 本条件では外挿域に触れない。ただし NASA-9 下限 200 K との余裕は
  47 K で、$T_t$ が下がる条件 (800 K なら出口 198 K) では踏むので、`GasSemiPerfect` に
  「200 K 未満は cp を凍結する」オプションを将来の安全側既定として検討 (未実装)。
- `2026-08-17` — **R/L スタディを TP (正本) で再走** (run_0031–0038、CPG 代替と同 8 点):

  | 設計点 | CPG(γ\*) | **TP** |
  | --- | --- | --- |
  | R1.5/2/3, L_U6, L_c=max | 0.328/0.306/0.256% | 0.108/0.117/0.115% |
  | R1.5/2/3, L_U6, L_c=8 | 0.436/0.523/0.681% | 0.146/**0.084**/**0.091%** |
  | R2, L_U4/9, L_c=max | 0.296/0.302% | **0.386**/0.102% |

  **結論の更新**: (i) **L_c=8 は TP では最良級** — CPG の「R≥2 でゲート外」は熱力学不整合の産物で
  撤回。全長 −0.35 m で軸 M は落ちない。(ii) **L_U=4 は TP でも明確に悪い** (0.39%、出口 M 4.216)。
  (iii) R は 1.5–3 で有意差なし。**推奨 = R2–3 / L_U6 / L_c=8** (全長 3.9 m)。TP は入口 BC の床
  (rms_ro 3e-2、軸 M 変動 3–4e-3) があり ~0.1% 以下の差は床と同程度・VERDICT NOT CONVERGED を併記。