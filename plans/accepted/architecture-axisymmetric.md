...
- 理論: [`methods/axisymmetric/theory.md`](../../methods/axisymmetric/theory.md)
  (B 流儀、圧力ソース導出、Roe 4 波固有分解)。
- 実装: [`methods/axisymmetric/implementation.md`](../../methods/axisymmetric/implementation.md)
  (フラグ・$r$ 重み付け箇所・カーネル設計・ソース対応表)。
- 既存 3D Roe: [`methods/convection/theory.md`](../../methods/convection/theory.md) と
...
支配方程式は `methods/axisymmetric/theory.md` を参照。要点のみ:
...
1. ✅ `methods/axisymmetric/theory.md` 作成 (B 流儀導出、圧力ソース、Roe 4 波)。
2. ✅ `methods/axisymmetric/implementation.md` 作成 (フラグ、$r$ 重み付け箇所、
   ソース対応表)。
3. ✅ `methods/index.md` に `axisymmetric` 行を追加。
...
28. `methods/axisymmetric/*` と `methods/index.md` の整合性確認 (実装で乖離が出た
    箇所を docs に反映)。
...
- 既存ケース・実行手順への影響: `isAxisymmetric` 未指定で完全互換。新フラグ
  を立てない限り従来通り。
- ドキュメント: `methods/axisymmetric/*`, `methods/index.md` を整備済 (Phase 1A)。

## 8. 完了条件

- [x] 関連 [`methods/axisymmetric/theory.md`](../../methods/axisymmetric/theory.md) 作成
- [x] 関連 [`methods/axisymmetric/implementation.md`](../../methods/axisymmetric/implementation.md) 作成
...
- `2026-06-02` — 本 plan を完了扱いに更新。
  - `methods/axisymmetric/theory.md` の phase 記述を実装現状に合わせて更新。
...
- ~~軸近傍セルにおけるソース項剛性の implicit 処理 (`res_roUy += p A_planar` の
  ...
  **実施済 (2026-06)**: block DPLUR (`timeIntegration==11`) の `implicit_defect_correction_block_d` に
  ...
  [`gpu-implicit-plan.md`](../active/gpu-implicit-plan.md) と [`methods/axisymmetric/`](../../methods/axisymmetric/)。
