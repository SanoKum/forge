...
- **status**: `done`
- **related_docs**:
  - `methods/time_integration/theory.md`
...
- 輸送項 point-implicit 対角 $\Lambda^{T}_\phi$ の定義・定常解不変性・検証は
  [`methods/time_integration/theory.md`](../../methods/time_integration/theory.md) §"輸送項 (移流+拡散) の point-implicit 対角"。
  segregated point-implicit 本体は同 §"スカラー (k/ω) の陰解法"。実装は
  [`methods/time_integration/implementation.md`](../../methods/time_integration/implementation.md) §`applySSTPointImplicit_d`。
...
- docs: `methods/time_integration/theory.md`・`methods/time_integration/implementation.md`（#2 実装時に粘性節追記）。
  新規 docs ファイルは作らないため `methods/index.md` は不変。

## 8. 完了条件

- [x] 関連 `methods/time_integration/theory.md` 更新済み（輸送項 point-implicit $\Lambda^{T}_\phi$）
- [x] 関連 `methods/time_integration/implementation.md` 更新済み（`transport_diag`）
