...
- **NS v1** (`run_0073_a13_v1`, coarse 中継 `run_0072_a13_coarse`。品質 PASS・
  conv ALL PASS・ALL STEADY・NaN 0): ‖ΔM‖∞ **0.582%** (A12 v1 0.533%)、
  **overshoot +0.03%** (A12 +0.30%、10 倍改善)、rms 0.0131。ΔM プロファイル:
  上流の正ずれ (+0.012→+0.009) と出口側 (+0.013→+0.002) が消え、
  **残るのは x≈5.4 の谷のみ** = Euler と同源の設計側残差 → law 帰還の対象。
  Codex の順序論 (物理スロート → law 帰還) が正しかったことの実証。
...
0.24→0.95% 悪化させた。$n_{cp}\ge48$ なら差は縮むが、そこまで来ると「補間 165 点」との
自由度差が小さく LSQ の意義 (低自由度表現) が消える。

**結論**: 補間 5 次 B-spline を本流として維持。`LSQBsplineCFDWall` は `wall_repr: lsq` で
残置 (将来 CAD 出力・最適化 DOF が要るとき、n_cp を上げて再評価する出発点。
「エクスポート層のみ」は CAD≠CFD 形状となるため引き続き不採用)。
