...
- 新規: `cuda_forge/thermo_d.{cuh,cu}`, `methods/thermophysics/{theory,implementation}.md`, `case/13.nozzle_H/validate_tpgas.py`。
- 変更: `cuda_forge/dependentVariables_d.{cu,cuh}`, `cuda_forge/convectiveFlux_d.{cu,cuh}`, `cuda_forge/gasProperties_d.cu` (TP は無改変)、`cuda_forge/CMakeLists.txt`, `input/solverConfig.{hpp,cpp}`, `main.cpp`, `methods/index.md`。
...
- [x] `.github/plans/README.md` / `methods/index.md` 更新
- [x] 本 plan を `done` 化し §9 に変更ログ
- [ ] (後続) §10 残課題 — 組成依存エネルギー BC・RANS+TP 検証・ROE 段階B 等

## 10. 残課題 (後続。M1-M4 のコア外の改良)

- **【現状の最重要課題】Cutler 検証の定量照合が未実施**: `case/28.cutler_coaxial_jet` の binary [He,N2] (`run_0030`) / ternary [He,O2,N2] (`run_0032`) はいずれも **clean かつ定常・収束した物理的に妥当な場** (T max ~388K, 残差 ~2e-7, ΣY=1, 二流/三流せん断構造) を得ており、シミュレーション側のプロファイル (中心軸 He モル分率減衰・He ポテンシャルコア長・径方向 X_He/|U| プロファイル) は `extract_cutler_profiles.py` で抽出済み。**ただし、これらが定量的に正しいかを判定する「外部基準との照合」が未実施** — すなわち ① 実験データ (Cutler AIAA-99-3588 / RELIEF・GSAS survey の He モル分率・pitot 圧・全温プロファイル) のデジタイズ重畳、または ② 既存の数値解析 (公開 CFD 結果・文献値、例: VULCAN 等での同ケース計算) との比較、のいずれも行えていない。現状の主張は「物理的に妥当・自己整合・定常」までであり、**「実測/既存解析と定量的に一致する」という検証レベルには未達**。次アクション: `cutler_exp_centerline.csv` (列 `x_over_D,X_He` 等) を入手・整備して `extract_cutler_profiles.py` の重畳照合を実行する、もしくは文献の数値結果と突き合わせる。
...
- **多成分 implicit 結合の恒久修正 (要因2)**: `physProp.thermoHrefTemp` (sensible-enthalpy datum, 2026-06-19) で安定 `cfl_pseudo` 上限を 1→2 に上げた (部分緩和)。続けて **案B (緩和整合 scalar-DPLUR, `speciesImplicitCoupling`)** を実装・検証 → **無効と判明** ([thermophysics-species-implicit-coupling.md](thermophysics-species-implicit-coupling.md), case/16 README run_0110〜0120)。理由: ① matched≡off で cfl 上限が一切動かない、③ 組成完全一様で `res_roY≈0`=移流緩和は無関係 (H2O ペナルティは EOS エネルギー再構成側)。**【2026-06-20 訂正】** 当初②で「流れ/SST 律速」としたが誤り。CPG/TP 対照 (case/16 README「ノズル CFL 上限」run_0121〜0155) で**真の cfl 律速・残差プラトーは `thermalMethod=2` (TP/NASA-9) 熱力学**と判明 — CPG は同一ノズル+SST で cfl100 まで rms 1e-12 収束、TP は cfl2 でも 1e-7 プラトー・cfl≥5 発散。前回「単成分 N2 も cfl4 発散」とした run はそれ自体 TP だった (誤帰属)。**最上位の恒久修正 = block-DPLUR Jacobian の TP EOS 微分整合化** (CPG 閉形式 `chi=(γ-1)/sonic` の凍結 γ[ic] 流用をやめ ∂P/∂(ρe) 等を TP 実微分で評価)。要因2 (組成-エネルギー結合) はその下位。**【2026-06-20 実装完了】** 一般EOS固有系 (音響 `Ht±cU_n`・接触 `Ht−c²/κ`・左密度に `χ=c²−κh`) を閉形式で実装 ([time_integration-general-eos-jacobian.md](time_integration-general-eos-jacobian.md), case/16 README run_0161〜0166) → **TP cfl 上限 2→≥100・残差プラトー 9.6e-8→4e-11 (3.6桁)・CPG ビット不変**。残る ~4e-11 床は EOS 反転側 (別課題)。
...
- `2026-06-10` — **Track B: Cutler 同軸ジェット発達場の clean 化 (M7 が遠方場 entrainment 病理を解消) + 定量照合ツール整備**。
  - **病理の発見・診断**: 既存の発達計算 (`run_0013` ternary / `run_0021` binary openbc) の場は **NaN なしだが非物理** — 外側境界 (r≈48-60mm, x≈60-230mm 下流) の空気が **T≈2100K・ρ≈0.16** に暴走 (全域の 12-23% が T>500K)。coflow 空気の全温 Tt=181×(1+0.2·1.8²)=298K なので空気は物理的に 300K を超えられず、明確なエネルギー異常。**根本原因 = pre-M7 の `outlet_statPress` entrainment**: 遠方場を `outlet_statPress` (backflow で ambient 流入) にしていたが、TP 再構成が `sp[0]`=He 固定だったため流入空気を **He 密度** (ρ=P/(R_He·300)=0.163) で構成 → 内部の空気セルが He 並み低密度を継承 → T=P/(ρ·R_N2)≈2100K。**まさに M7 が直す対象**。
  - **M7 による healing 検証**: `run_0021` の発達場 (pre-M7, 外側 T~2100K) を M7 バイナリで warm-start (`run_0029`→`run_0030`)。T>1000 セルが **17103 → 9135 (3k 步) → 603 (10k 步) → 0 (20k 步)**、外側境界が **T=299-301K・ρ=1.10-1.14 (= 正常 ambient)** へ完全回復。`res_20000` で **T max=387K・残差 rms_ro=1.66e-7** の clean な収束場を得た。M7 が単なる整合性改善でなく **検証場の物理妥当性そのものに必須**だったことを実証。
  - **定量照合ツール** (`case/28/extract_cutler_profiles.py`): res_*.h5 + 内蔵メッシュ (XDMF Mixed quad) から中心軸 He モル分率減衰・He ポテンシャルコア長・x/D ステーション径方向プロファイル (X_He, |U|, Tt) を抽出し CSV+PNG 出力 (`cutler_exp_*.csv` があれば重畳)。**clean 場 (`run_0030/res_20000`) の結果**: He ポテンシャルコア **x/D≈17.5** (X_He=0.99)、中心軸 |U|=**1271 m/s** (=Mach1.8 He ✓) を x/D~17 まで保持し下流で減衰、二流構造 (噴流 1271 / coflow 490 m/s) と shear-layer 混合を正しく解像。
  - **3 成分 [He,O2,N2] へ拡張 (`run_0031`→`run_0032`)**: binary と同手順で ternary 粘性+RANS-SST+openbc を構成 (bcond=`run_0022` の openbc 遠方場 + ternary 入口、乱流入口 k/omega を RANS 用に更新; solver=`run_0013` の viscMethod=2/SST/3-species/化学種拡散)。`run_0013/res_20000` (slip 遠方場の汚染場) から M7 で warm-start すると **10k 步で完全 healing** (T max 2187→389K, T>500 セル 9860→0, 外側 T=299-301K/ρ=1.12-1.16, 残差 rms_ro=1.85e-7)。binary より速く収束。プロファイル: He-O2 ジェットは **X_He=0.95** (=95/5 vol のモル分率) を中心軸で保持、**He ポテンシャルコア x/D≈20.7** (99%, binary 17.5 より長い — He-O2 は密で遅く混合弱め), 中心軸 |U|=**1100 m/s** (He-O2 jet), 三流せん断構造 (jet 1100 / coflow 490) を解像。`extract_cutler_profiles.py` のコア定義を出口モル分率正規化に更新し binary/ternary 双方対応。
  - **残 (現状の最重要課題, §10 冒頭参照)**: binary/ternary とも物理的に妥当な clean 収束場は得られたが、**実験データ (RELIEF survey) または既存の数値解析 (公開 CFD・文献値) との定量照合が未実施**。現状は「自己整合・物理妥当・定常」までで、「実測/既存解析と定量一致」までは未検証。実験プロファイルのデジタイズ (`cutler_exp_centerline.csv` 等) もしくは文献数値の入手後に `extract_cutler_profiles.py` で重畳照合する。
- ビルドは Docker dev image (`forge-solver:cuda-dev`) で実施 (native は libyaml-cpp 0.8 + hdf5 + nvcc 12.0 が揃い、速度計測は native で実施可能)。

- `2026-06-18` — **16.wys 多成分 TP 非収束の根本原因 3 件を特定 (診断のみ。恒久修正は要因2が未実装)**。case/16.nozzle_wys/README.md「多成分 TP 発散の再検証 run 一覧」(run_0069〜0094) に検証 run。
  - **要因1 (修正済)**: IC 生成器 `case/16.nozzle_wys/gen_ic_2sp_from_n2.py` の運動エネルギー誤り (`ek=0.5*ΣroU²/ro` → 正: `/ro²`)。`roe=ro*(e_mix+ek)` の ek が質量当たりでなく係数 ρ だけ過大→solver 読み戻しで高速域 (出口) の `intE` が過小→T が 200K 床に張り付き多数セル T<200K→発散。`res_0.h5` で生成時 T=288-503K が読み戻し 9466 セル T<200 になることを確認。修正後 `res_0` 整合・初期エネルギー残差が単成分並み。
  - **要因2 (回避策のみ・恒久修正未実装)**: 高温 (Tt=500K, 全>200K, 単成分は安定) でも N2+H2O は発散。**O2 は無害 (h_O2−h_N2≈0)・H2O 固有 (h_H2O≈−13.4 MJ/kg)**。Y_H2O=1e-6 でも発散 (入口 BC が 0.01095 を課す)。症状=「H2O が入ったセルの T が偽上昇」。機構 = implicit で `roe` を block-DPLUR、`roY` を別 point-implicit で更新→擬似時間緩和ミスマッチ→`roY` 変化に `roe` 非追従→Newton で T ジャンプ、H2O の |h| が増幅。**cfl_pseudo 依存を確認 (=10 step2 / =2 step14 / =1 安定 / =0.1 安定)。回避策: 多成分 TP は `cfl_pseudo ≤ 1`** (`run_0093` で N2+H2O 1% が hot+cfl_pseudo=1 で 400 step 完走)。**デッド `species_energy_correction_kernel` (roe+=Σ(roY−roYN)h_s) は前任が書いたが main 未配線。配線すると二重計上で O2 まで発散** (`run_0091`)→対流流束は既に化学種エンタルピーを整合輸送している証拠。恒久修正は species を 5×5 block に同梱 or 緩和整合が必要 (未実装)。
  - **要因3 (cold 固有・0acca05 既知)**: 全 species Tlo=200K。Tt=286 (出口159K) の cold は要因1修正+cfl_pseudo=1 でも step13 発散 (`run_0094`)=sub-200K NASA-9 外挿。cold 実ケースは CPG (thermalMethod:0) か thermo 低温拡張が必要。
  - **棄却**: float32 精度 (double 同一軌跡)・Fick 拡散エネルギー結合 (`res_roe+=Σh_s J_s*` を切っても不変)・凝縮 (off で発散)。
- `2026-06-19` — **要因2 への打ち手: エンタルピー基準オフセット (sensible-enthalpy datum) を実装・検証 (部分的に有効)**。
  - **動機**: 要因2 (多成分 implicit 結合不安定) は H2O の絶対 (生成込み) エンタルピー `h_H2O(298K)≈−13.4 MJ/kg` が roe(block-DPLUR)/roY(point-implicit) 緩和ミスマッチを増幅し Newton で T ジャンプを起こす機構。**非反応流では各種のエンタルピー基準を任意の定数だけ平行移動してよい** (e_mix と対流/拡散エンタルピー Σh_s J_s* の基準移動が連続式で厳密に相殺) ので、`h_s(Tref)=0` へ揃えて桁違いの生成エンタルピーを除けば増幅が減るはず、という仮説。
  - **実装 (一点改変で全経路整合)**: config `physProp.thermoHrefTemp` (>0 で有効, 既定 0=従来絶対基準・ビット不変)。`thermo_init_db` で各種の NASA-9 係数 `a[7]` に `Δa7=−h_s(Tref)/Ru` を両温度域へ加算 (h_molar の定数項が `Ru·a7` ゆえ全 T で一定オフセット・Tmid 連続)。係数に焼き込むため `thermo_h/cph/T_from_e/混合則/SLAU 面エンタルピー/BC` が自動的に同一基準になり、エントロピー `a[8]` は不変。Python IC 生成器 `gen_ic_2sp_from_n2.py` も第6引数 `Href_Tref` で同一基準にオフセット。
  - **検証** (`case/16.nozzle_wys/run_0101〜0109`, hot N2+H2O 1%, 全>200K, 同一場・同一 BC, native full rebuild): ① **安定 `cfl_pseudo` 上限が 1→2 に向上** (絶対基準は cfl2 で step6 発散、オフセットは cfl2 で 400step 完走)。高 cfl も発散ステップが一貫後退 (cfl4: step3→7)。② **物理不変を実証**: step0 再構成 T/P/ρ/Ux が絶対 vs オフセットで機械精度一致 (T rel 1.2e-7)、roe は `≈ρ·Y_H2O·h_ref(H2O)` だけ平行移動。`roe/ρ` が −86〜−19 kJ/kg(負・桁落ち) → +61〜+127 kJ/kg(良条件) に改善。③ **限界**: 上限は 2× 止まり (`cfl_pseudo≥4` は依然発散)・残差はプラトー (NOT CONVERGED, 単成分 run_0056 と同等)。**要因2 の構造的ミスマッチ自体は未解消で、オフセットは増幅振幅を下げるのみ**。
  - 図: `case/16.nozzle_wys/thermo_href_compare.png` (cp/h vs T と各種オフセット), `href_cfl_ceiling.png`。
