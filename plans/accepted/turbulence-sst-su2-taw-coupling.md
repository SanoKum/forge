...
- **ω irep ピンの検証結果 → 正式化見送り (2026-08-11 追試)**: y+1 壁解像 (run_0052, 解像
  ...
  **SU2 wf 0.985 / forge 生産 (mode 3, ピンなし) 0.945 / forge+ω ピン 1.237**。ω ピンは第一内点
  ...
  必ず ρ_rep 基準で比較すること。付随知見: (a) forge 生産の τ_w は y+1 内部基準比 −5.5% で健全
  (**統一評価 `wall_tau_eval.py` では −4.51%**。「真値」ではなく**内部基準**である点も訂正)、
...
- **壁 μt の SU2 5 倍差 — limiter 経路へ局在化 (2026-08-11、内訳は未確定)**:
  外部レビュー (codex) の指摘で 2 度訂正した。**root cause 特定完了ではない**。
  - forge の `sst_eddy_viscosity_d` は SU2 `SetEddyViscosity` と**式は完全一致**
    ($\mu_t=\rho a_1 k/\max(a_1\omega, S F_2)$、SST_OPTIONS=NONE → v8 は V2003 = strain magnitude
    で種類も一致)。格納値はこの式と厳密一致 — 実装バグではない。
  - **5 倍差は壁ノード W に局在** (irep では forge/SU2 = 0.68〜0.79 で大差なし)。
  - **確定した内訳 (格納 μt から実効分母を逆算、勾配再構成に非依存)**:

    | node 6564 | ρk/ω (limiter 前) | 実効分母 = S·F2 | a₁ω | L = S·F2/(a₁ω) | atten | μt |
    | --- | --- | --- | --- | --- | --- | --- |
    | forge | 3.56e-3 | 6.45e6 | 1.64e6 | **3.93** | 0.254 | 9.05e-4 |
    | SU2 | 4.08e-3 | 1.00e7 | 4.58e5 | **21.9** | 0.0456 | 1.86e-4 |

    **limiter 前の $\rho k/\omega$ は 13% 差しかなく、μt の 4.87 倍差は attenuation 比 5.58 倍が
    ほぼ全部を占める** (0.873 × 5.575 = 4.87)。3 ノードとも同傾向。
    L 比 (0.179) の分解: **S·F2 寄与 0.643 × a₁ω 寄与 0.279** →
    **支配項は S ではなく ω (forge の壁 ω が 3.6 倍大きい)**、S·F2 は副次 (forge が 1.55 倍小さい)。
    k・ω が同方向に ~3.4 倍ずれて比では相殺するが、limiter は $a_1\omega$ と $SF_2$ の
    **絶対スケール比較**なので相殺しない、という構造。
  - **SU2 側の参照値自体に疑義 (要追加調査)**: ω_w 公式 $\sqrt{\omega_{vis}^2+\omega_{log}^2}$ は
    両コード同一。y=幾何隣接距離で評価すると forge の格納 ω は公式の 1.33 倍 (forge は
    $y_{eff}$=`wall_dist[irep]`≈0.75×幾何距離を使うため — 内部整合はしている定義差)。一方
    **SU2 の壁ノード格納 ω は自分自身の公式の 0.35 倍**で、しかも SU2 の第一内点 ω は公式の
    1.14 倍 (整合)。`SetTurbVars_WF` は壁と Normal_Neighbor に**同一値**を書くはずなのに
    実測では 3.2 倍食い違う (wall 1.48e6 / neighbor 4.77e6)。`GetWall_Functions()` 時は
    標準 BC (60ν/β₁y²) を通らず即 return するのでそれも説明にならない。
    → **SU2 の壁 μt=1.86e-4 は未収束場の反復位相/緩和の産物である可能性があり、
    「合わせるべき真値」として未検証**。これまでの調査は暗黙にこれを truth 扱いしていた。
  - **【誤りの記録①】** 初回究明では「壁ノード W の Green-Gauss セカント勾配が局所せん断より
    5.4〜7.6 倍過大」を root cause と誤報告。勾配歪み自体は実在 (解析式
    $\text{secant}/\text{local}=\kappa u^+(y_I)$ で確認、gradLSQ コメントにも既知課題として明記)
    だが、S は limiter 分母なので過大な S は μt を**下げる**方向。5 倍差の説明にならない。
  - **【誤りの記録②】** 次に「壁ノード W に k ピンがないこと」を root cause と誤報告。
    しかし (i) limiter 前 ρk/ω は 13% 差しかなく k 単独では説明不能、(ii) **run_0067 で壁 k ピンを
    実際に試した結果 node 6564 は k 12374→12954・μt 9.05e-4→9.47e-4 と SU2 から僅かに
    遠ざかった** (実測で否定済み)。よって「既存 Reichardt 版 k ピンの W への延長」は有力候補から外す。
  - **現時点の正確な結論**: 5 倍差は壁ノード W の **limiter 入力 $SF_2/(a_1\omega)$ の差**に局在し、
    その内訳は ω 支配 (3.6x) + S 副次 (1.55x)。**S・F2・ω それぞれがなぜずれるかは未確定**。
  - **① SU2 参照値の妥当性判定 → 「5 倍差」は追う対象ではないと結論 (2026-08-11, follow-up 完了)**:
    y+1 壁解像 (run_0052) を真値アンカーに、物理的に意味のある量へ引き直して決着した。

    | 比較点 | y+1 真値 | forge y+30 | SU2 y+30 |
    | --- | --- | --- | --- |
    | **壁ノード上の μt** (x=2.3mm) | **9.1e-12 (≈0)** | 9.05e-4 | 1.86e-4 |
    | 第一内点 μt (x=2.3mm, y=1.06e-4) | 2.20e-4 | 8.83e-4 (**4.02x**) | 1.12e-3 (**5.11x**) |
    | 第一内点 μt (x=3.1mm) | 2.16e-4 | 8.63e-4 (3.99x) | 1.10e-3 (5.07x) |
    | 第一内点 μt (x=40.8mm) | 6.89e-4 | 3.46e-4 (0.50x) | 5.08e-4 (0.74x) |

    - **壁ノード上の μt には物理的な参照値が存在しない**: no-slip 壁では真値が厳密に ~0
      (y+1 解像で 9.1e-12)。forge 9.05e-4 も SU2 1.86e-4 も**どちらも純粋なモデル人工値**であり、
      両者の比 4.9 倍は「2 つの任意なモデル値同士の比」でしかない。合わせるべき根拠がない。
    - **μt が物理的意味を持つ第一内点で比べると、forge は SU2 より真値に近いか同程度**
      (スロート近傍 4.0x vs 5.1x、下流 0.50x vs 0.74x)。y+30 壁関数メッシュが近傍で μt を
      4〜5 倍過大評価するのは**両コード共通の解像度起因**で forge 固有の欠陥ではない。
    - **意味を持つのはモデルが生む流束だけ**であり、それは既に検証済み:
      τ_w は y+1 真値の 0.945 倍 (SU2 0.985 と同等に健全)。mode 3 により熱経路からは
      壁 μt が完全に外れている。
    - **結論: 本 follow-up は「欠陥ではない」としてクローズ**。今後 SU2 の壁 μt を
      合わせ込み目標にしないこと。近壁 μt 精度を本気で問題にするなら、対象は
      「第一内点 μt vs y+1 真値」(両コードで 4〜5 倍過大) であり、それは壁関数の解像度・
      定式化の課題として別テーマになる。
  - **副次的に判明した方法論差 (記録のみ)**: SU2 の既定勾配法は **WEIGHTED_LEAST_SQUARES**
    (`CConfig.cpp` の `NUM_METHOD_GRAD` 既定、本 cfg は未指定) に対し forge は Green-Gauss
    (`gradLSQ: 0` 既定)。高アスペクト比の壁セルで GG と LSQ は近壁勾配が大きく食い違うため、
    S·F2 の 1.55 倍差の少なくとも一部はこの方法論差で説明される。forge 側の LSQ は
    [`discretization-lsq-gradient.md`](../active/discretization-lsq-gradient.md) で開発中。
  - **未解明のまま残す点 (実害なしのため深追いしない)**: SU2 の壁ノード格納 ω が SU2 自身の
    `SetTurbVars_WF` のどの解釈でも再現できない (公式値 4.2e6 / 第一内点 4.77e6 / 緩和合成でも
    4.4e6 以上のはずが、格納値 1.48e6)。SU2 を計装できないため原因特定は不可能で、かつ
    上記のとおり参照値として使わないと決めたので追跡を打ち切る。
