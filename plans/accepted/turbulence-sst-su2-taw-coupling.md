...
- **壁 μt の SU2 5 倍差 — limiter 経路へ局在化 (2026-08-11、内訳は未確定)**:
  ...
  - **次に確認すべきこと**: ① SU2 の壁 ω が公式値の 0.35 倍になる理由 (緩和・反復位相・
    出力段の別処理) を SU2 側で特定し、参照値としての妥当性を先に判定する。
    ② forge の $y_{eff}$ 定義 (`wall_dist[irep]` = median-dual 距離) が壁法則の y として
    適切か (幾何隣接距離との 0.75 倍差が ω に直接効く)。③ S の再構成値を両コードで直接比較。
    ④ これらが片付いてから対応方針 (EddyViscWall 完全移植 / y 定義の見直し / 現状維持) を決める。
    **既存 k ピン延長と EddyViscWall 完全移植は別物として区別すること**。
    着手する場合は独立 plan として起票し、y+1 真値アンカーの τ_w 比較で検証する。
