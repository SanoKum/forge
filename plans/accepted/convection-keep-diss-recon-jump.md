...
- 2026-07-19: **keepDissJump=2 (sign-property クリップ, TeCNO 型) 追加 — 証明付き ES 化**。
  - 動機: jump=1 は再構成が行き過ぎた面で特性成分の符号が反転し局所エントロピー生産が可能
    (証明なし・実測担保のみ)。事前測定: 符号反転は (面,波) の 27-37% で発生するが、
    負寄与は総散逸の 1-2% に過ぎず、minmod クリップ後の散逸総量は jump=1 とほぼ同一。
  - 実装: 再構成ジャンプと生ジャンプの両方を特性空間へ射影し、成分ごとに
    rd_used = minmod(rd_rec, rd_raw) — 各波で rd_used·rd_raw ≥ 0 が構造的に成立し
    Σ S_k λ_k rd_used·rd_raw ≥ 0 (エントロピー散逸性) が**証明付きで復活**。
    再構成不発の面は rd_rec == rd_raw で恒等。追加コストは log 2 回+射影 5 本/面。
  - 検証 (node): L1 市松 6.99e-6 (生 6.78e-6 と同等; case/35 run_0042) /
    L3 64³ K/K0(10) −1.4% (jump=1 −0.8% と生 −2.7% の間)・ピーク t*=8.96 維持・層流期 jump=1 と同一
    (case/09 run_0041)。**推奨既定を jump=2 に更新** (jump=1 は証明なし最軽量として残置)。
- 2026-07-19: **訂正**: L3 の「WALE 併用」は WALE 不活性 (ILES) だった
...
- 2026-07-29: **既知の制約を定量化 (外部レビュー指摘の検証)**: 再構成ジャンプの O(h³) 選択性は
