...
- **related_docs**:
  - [`methods/convection/theory.md`](../../methods/convection/theory.md) (低マッハ前処理節)
  - [`methods/convection/implementation.md`](../../methods/convection/implementation.md) (`lowMachPrecond`)
  - [`methods/time_integration/theory.md`](../../methods/time_integration/theory.md) (前処理固有値・局所時間刻み)
...
- 散逸スケール是正の理論: [`methods/convection/theory.md`](../../methods/convection/theory.md) の
  「低マッハ前処理 (Weiss–Smith 型・散逸スケール是正)」節 (SLAU2 節の直後、HLLE 節の前)。
- 前処理固有値・局所時間刻み: [`methods/time_integration/theory.md`](../../methods/time_integration/theory.md) の
  「低マッハ前処理固有値 (Weiss–Smith)」節と「局所時間刻み」節。
- 実装対応: [`methods/convection/implementation.md`](../../methods/convection/implementation.md)
...
- docs: `methods/convection/{theory,implementation}.md`, `methods/time_integration/{theory,implementation}.md`
  (更新済)。`methods/index.md` は構成不変につき確認のみ。
- 既存ケース: `lowMachPrecond`・`lowMachThornber` 既定 0 のため挙動不変。

## 8. 完了条件

- [x] 関連 `methods/convection/{theory,implementation}.md` 更新済み (収束ベース所見・Phase 2 不採用を反映)
- [x] 関連 `methods/time_integration/{theory,implementation}.md` 更新済み (固有値/setDT 前処理は不採用と明記)
...
- [x] **Phase 4 (a) $\Gamma^{-1}A$ 完全前処理** (`lowMachPrecond=2`) — 実装・検証完了。**低マッハ自励振動を根治** (ε=0.05 で振幅 0.882%→0.087%・定常収束。§9 `2026-06-09`)。時間項 $\Gamma_c$+物理フラックス FVS+$\Delta\tau'$。**Sherman-Morrison で FP64 排除し物理 block と同速** (per-step 16.9ms)、収束加速も net win (深収束で 2.2×)
...
- `2026-06-09` — **Phase 4 根治テスト成功。低マッハ自励振動を根治 (採用確定)**。`case/23.axi_nozzle` 20k step、
  ...
  - **残課題 (任意)**: per-step の倍精度コスト低減 (→ Sherman-Morrison で完了)、β=1 回帰 (→ 完了 PASS)、
    ε 最適化 (→ ε≈0.05 と確定)、2D 低速妥当性 (→ flat_plate で完了 PASS)。すべて後続 `2026-06-09` エントリで対応済。
...
- `2026-06-09` — **Sherman-Morrison 化で FP64 を排し、前処理カーネルを物理 block とほぼ同速に高速化**。
  ...
  - **収束加速への波及 (再測定・net win に転化)**: SM 版で per-step が m1 と完全同速 (15.7 vs 15.6ms) になり、
    安定限界も m2(SM) ~7-9 (cfl5/7 安定・cfl10 発散) と確認。同 ε=0.15 で m1@cfl1 と m2(SM)@cfl5 を比較
    (per-step 同速ゆえ step=wall-clock)、「rms_roe が閾値を最初に下回る step」: <3.0 で 1.4×、<2.0 で 1.3×、
    <1.5 で 1.6×、**<1.0 で 2.2×** (深く収束させるほど加速大。m1 は limit-cycle フロア手前で鈍化、m2 は高 cfl で
    押し下げ継続)。→ **当初「ブレークイーブン」だった収束加速は FP64 排除で net win に転化** (根治が主・加速も従で得)。

- `2026-06-09` — **任意の残課題3件を完了 (β=1 回帰 / ε 最適化 / 2D 低速妥当性)。いずれも十分収束で判定**。
  1. **β=1 ビット回帰 (PASS)**: `precondEps=1.0` で全セル $U_r=c\Rightarrow\beta=1\Rightarrow\Gamma_c=I,\ \Delta\tau'=\Delta\tau$ となるので
     =2 は =0 に帰着するはず。`case/23.axi_nozzle` で =0 vs =2(ε=1.0) を 1000 step 比較、残差の最大相対差 ~3e-5〜1e-4
     = atomicAdd/float 丸めノイズ床 (OFF 回帰の run 間ノイズと同オーダー)。系統差なし → SM 前処理カーネルは
     前処理 OFF で正しく baseline に一致。
  2. **ε 最適化**: =2 で ε∈{0.03,0.05,0.08,0.10,0.15} を 20k・4k–20k 包絡で評価 (chamber std/mean mean):
     0.03→0.097%, **0.05→0.087% (最小)**, 0.08→0.109%, 0.10→0.126%, 0.15→0.333%。全て NaN なし・M_max~6.03 不変。
     **振幅単独の最適は ε≈0.05** (スイートスポット。0.03 は微増、≥0.08 単調悪化)。
     **ただし ε↔安定 cfl_pseudo の結合に注意 (単一ノブ)**: ε を下げると aggressive な `c'` 散逸を陰解法が支える
     安定 cfl_pseudo も下がる。axi_nozzle (=2) のフロンティア (smoke で確認):
     | ε | 安定 cfl_pseudo | 振幅 | 性格 |
     | --- | --- | --- | --- |
     | 0.05 | ~2 | 0.087% | 最強根治・遅 |
     | 0.08 | ~3-4 | 0.109% | バランス |
     | 0.10 | ~3-4 | 0.126% | バランス |
     | 0.15 | ~7 | 0.333% | 速・穏やか |
     → 推奨は目的別: **最強根治** `ε=0.05,cfl_pseudo≈2`、**バランス** `ε=0.08-0.10,cfl_pseudo≈3`、
     **速さ優先** `ε=0.15,cfl_pseudo≈5-7`。いずれも baseline 0.882% を大幅改善。**絶対値は強超音速の axi_nozzle 固有**で、
     穏やかな亜音速 (flat_plate M=0.2) は同 `=2` で cfl_pseudo=50 級が回る (ケース依存)。
  3. **2D 低速妥当性 (PASS)**: `case/26.flat_plate_sst` (M≈0.2 乱流平板) を developed restart (run_0008 res_10000) から
     =0 と =2(ε=0.15) で 5000 step。両者 NaN なし・安定。壁法則ツールで Cf/u_τ を比較: x=0.3/0.6/0.89 で
