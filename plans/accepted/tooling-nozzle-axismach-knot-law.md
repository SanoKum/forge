...
- **$M_K$ 詳細感度 (M6/R3, 2026-08-17)**: $L_c$ 30/35/40/45/50/60 × $M_K$ 1.5–4.0
  (0.1刻み) の156点を600、代表を1200/2400で評価 (`sweep_axislaw_A_MK.*`)。全成立候補は単峰・
  内部flip/交差0。最小壁角は $L_c=45$ で $M_K=2.6$ (14.045°, margin 1.841°)、50で2.7
  (12.739°, 2.534°)。既定2.5との差は小さく、最小sin角は各6/13%低下。CFD前なので既定は2.5維持。
- **最短ロバスト境界 (M6/R3, 2026-08-17)**: hard gate・単峰・$\min(\mu-\theta)\ge1^\circ$・
  最小sin角≥0.02を制約に、閉包後端$x_F$を最小化 (`optimize_axislaw_A_shortest.*`)。600点で粗→
  0.01刻み探索、境界のみ1200点確認。600点の$L_c=39.03$は1200点でmargin 0.9975°に落ち、
  **最短合格は$L_c=39.05,M_K=2.76$** ($x_F=94.34865r_t$, θ_max 16.1281°、margin 1.00095°、
  最小sin角0.04177、内部flip/交差0)。2400点・CFDは未実施のため設計境界として扱う。
...
- 2026-08-17: $M_K$ 詳細感度を追加。設計上の微調整候補は $(L_c,M_K)=(45,2.6),(50,2.7)$ だが、
  改善は小さくCFD未実施のため生産既定 $M_K=2.5$ は変更しない。
- 2026-08-17: 評価順序を品質制約下の$x_F$最小へ修正。600/1200点の境界探索で
  $(L_c,M_K)=(39.05,2.76)$を最短ロバスト設計境界とした。CFD前なので生産既定は未変更。
