...
## 2.12 追補 (2026-09-05): TP 亜音速出口の特性構成が γ を混用し低マッハ出口で偽逆流を課していた — 修正済

case/48 Cabra (coflow 3.5 m/s, TP 9 種, node) で出口列全体が step 数十で逆流 (−50〜−150 m/s) し圧力が
単調上昇→発散した。planar/軸対称・precond 0/2 で同一 (`run_0044`–`run_0046`) = 出口 BC 自体の欠陥。

- **真因**: `outlet_statPress_d` の亜音速分岐 (SU2 BC_Outlet 型特性構成) が `c_int = sonic[ic]` (TP の真の
  音速 √(γ_mix R T), γ_mix≈1.30) と `c_exit = √(ga_cfg·P_exit/ρ_new)` (config 定数 γ=1.35) を混用していた。
  P_int = P_exit でも c_int ≠ c_exit となり、Riemann 不変量 $V_{n,exit} = U_n + 2(c_{int}-c_{exit})/(\gamma-1)$ が
  **−65 m/s** ずれる (Δc≈12 m/s を 2/(γ−1)≈5.7 倍)。超音速出口 (全量外挿) では通らず、CPG では γ が同一なので
  無害 — TP × 亜音速出口だけが壊れていた (case/26 M0.2 平板の検証は CPG)。
- **修正**: TP では特性構成の γ を **セル局所 γ_mix(T)** (`var.c_d["gamma"]`) に統一し、`c_int` も同じ γ で
  √(γP/ρ) と再構成 (P_int=P_exit で厳密に V_n,exit = U_n)。超音速判定は従来どおり `sonic[ic]`。CPG・超音速は
  ビット不変。
- **検証**: `case/48 run_0047` (修正後): 出口列 3.5 m/s / 101.33 kPa を 200 step 保持 (修正前 `run_0044`:
  100 step で −51 m/s / 103.4 kPa)。単一種 N₂ ジェット `run_0048` 3000 step で P 101.0–102.0 kPa・T≤1045 K。
...
- `2026-09-05` — **TP 亜音速出口の γ 混用 (§2.12) 修正**: `outlet_statPress_d` の特性構成でセル局所 γ_mix を
  使用。低マッハ TP 出口の偽逆流 (case/48) 解消。同セッションで node × lowMachPrecond≥2 の境界ノード凍結も修正
  (plan time_integration-lowmach-preconditioning 変更ログ 2026-09-05)。
