...
- **残課題**: 残差床が baseline 比 6–12 倍 (rms_roOmega 0.14 vs 0.012)。集中箇所は出口リップ
  (node 13843–14297, y≈0.029) で、凍結場 ε 外れ値 (インレットコーナー x≈0 の +26〜+41 K と
  出口リップ node 14364 の +20.6 K) と同族のコーナー局所。スナップショットは 3000→12000 で
  ビット安定なので発散性ではなく局所リミットサイクル/停滞。ε コーナー処理 (淀み fallback の
  適用範囲) の改良が次の作業
