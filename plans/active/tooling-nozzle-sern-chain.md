...
6. **剥離が出る作動点をセットに入れるか** (2026-09-05 提起): TM X-71972 の 6 点 (最低 NPR 2.8 = M∞4 燃料遮断) では dv 箱の
   どの形状も剥離しない (§4.11 結果)。剥離制約を MOO で効かせるには NPR ≲ 2 の点、すなわち **M∞ ≲ 3 の燃料遮断 (降下・アボート)**
   が要る。TM の表には無いので (a) 簡易サイクルで M∞3 燃料遮断を外挿する / (b) 剥離制約は持たず `sep_frac` を台帳記録に留める、
   のどちらか。推奨 = (a) だが TM アンカー外の値になる点をユーザ判断とする。
