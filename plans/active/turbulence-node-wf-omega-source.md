...
→ **$\omega$ は局所 $P$–$D$ 平衡でほぼ完全に決まる**。交差拡散は無視でき輸送も 4%。
...
→ **E3 単独の効果は $Y_{10}$=0.8594 に近く、目標帯 0.94±0.02 には届かない見込み**。
E3 は「Dirichlet で殴らずに源項の整合で同じ $\omega$ 水準を得る」点に意味があり
(case/40 の過大化を避けられる可能性)、**目標到達を期待する実験ではない**。
残り (0.86→0.94) は $\omega$ 以外の機構、または $u_\tau$ を決める代表点速度側。
