...
5. **判定は定義非依存の運動量積分 $C_f=2(d\theta/dx+\mathrm{PG})$ を主指標にする**。
   良く収束した壁解像解では 4 定義 (解像勾配 / ソルバ出力 / 運動量積分 / Reichardt 逆解き) が
   **1.0% 以内で一致**する (SU2 `run_0049`) ので、これがパイプライン健全性の検証にもなる。
   **forge node 壁関数は運動量積分でも 0.759** = $\theta$ 発達そのものが 24% 遅く、
   壁応力の出力だけの問題ではない。SU2 壁関数版は場 (積分 1.003) は正しいが
   4 定義が 86% 開いており内部不整合 (比較相手にするには収束/実装確認が要る)。
