...
判定は §7.4 の 3 点セット。帯 x=[0.60,0.80]、同一 IC・20000 step、派生量 `STEADY`
(残差は全 run `NOT CONVERGED` = 本 case の構造プラトー)。

| run | (1) **W–I 実力** $C_f$/KS | (2) 運動量積分 | (1)/(2) | (参考) 壁法則 |
| --- | --- | --- | --- | --- |
...
| 効果 (応答量 = (1)) | 値 |
| --- | --- |
| 単純効果 pin (byp=0 / byp=1) | +0.0968 / +0.1181 |
| 単純効果 bypass (pin=0 / pin=1) | +0.0054 / **+0.0267** |
| **平均主効果 pin** | **+0.1075** |
| **平均主効果 bypass** | **+0.0160** |
| **相互作用** | **+0.0213** |
...
3. **回復率**: $Y_{11}$=0.876 は目標帯 0.94±0.02 に対し
   **中心 0.94 まで 66% / 下限 0.92 まで 74%** (基準を必ず明記する)。
...
- [ ] **$\omega$ 項別収支の診断出力 (§4.1) — 次のタスク。E3 の設計根拠にする**
