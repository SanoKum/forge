...
- docs: `methods/condensation/{theory,implementation}.md`, `methods/index.md`。
...
- `2026-06-16` — **TP gas (thermalMethod 2) + 凝縮を wys で検証 → 真因 = sub-200K の NASA-9 外挿不安定**。
  ...
  - **将来 TP を <200K で使うなら**: 下記の追加調査が要る。診断 run は `case/16` `run_0025`〜`run_0045`。
- `2026-06-16` (追記・**真因の訂正**) — **発散は NASA-9 外挿が原因ではない**ことが判明。追加検証:
  - **thermo クランプ/キンク棄却**: N2 の `Tlo` を 200→100K に下げ (159-286K も多項式・クランプ/キンク無し) ても
    `run_0044` は step14 で同一発散。N2 cp は 159-286K で 1039→1042 J/kgK と滑らか (差 0.3%)、キンクも微小。
    → **低温 thermo 自体は健全で原因でない**。
  - **s0 非整合は別の正当バグ (修正済)**: `thermo_s0_mass` が <Tlo で s0=const クランプ (h は線形外挿) で
    熱力学非整合だった。定 cp 整合の対数外挿 `s0(Tb)+cp(Tb)ln(T/Tb)` に修正。ただし wys は SLAU で内部に s0 を
    使わず、`run_0043` は step14 で不変 → **これも wys 発散の主因ではない** (Roe/等エントロピー BC の正しさ改善)。
  - **IC も棄却**: quasi-1D TP 等エントロピー IC を forge が読み戻すと T=[159.2,286.0] と意図に完全一致・EOS 整合
    1e-5 → 初期場は sub-200K でも整合。
  - **真の症状**: explicit 毎ステップ追跡 (`run_0045`) で **温度の成長振動** (Tmin 159→145→121→86→50K の過冷却 +
    Tmax→6000K の過加熱) が冷たい超音速出口で発生し入口へ伝播。Tt スイープの相関は「200K 閾値」でなく
    **絶対温度が低いほど速く育つ**性質 (Tt=500 安定 / 360 step24 / 286 step12)。
  - **結論 (更新)**: wys TP 発散は **TP 離散化 (フラックス/エネルギー) の冷・高マッハ域での数値不安定**で、
    thermo・IC・BC・精度のいずれでもない。特定のフラックス項までは未到達 (要フラックスレベルの計装)。
    実用上は **>200K で TP は安定動作**するため、ご本来の燃焼風洞凝縮では使用可、極低温 wys は CPG が正。
