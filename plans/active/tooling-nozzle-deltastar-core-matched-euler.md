...
非単調なら $\delta_{next}$ の平滑化スプライン λ を 1e-2 → 1e-1 → 1 → 10 と強めて再判定し、使った λ を CSV ヘッダと summary に記録する
...
- `2026-09-04` — **V2 pass 1 (`run_0023`, ω=1.0)**: ṁ 0.9999、出口面コア M +0.01〜0.02 %、固定点差 p10–p90 0.997〜1.002 (収束)、
  ...
  生産推奨 = 積分法初期壁 + 1 pass (pass 0 単独でも主ゲートは満たす)。V4 (case/42 M5 `run_0108`, case/44 `run_0030`: 積分法 pass 0) 投入。
