...
- `2026-09-05 (12)` — **切り分け ② (再)**: `run_0078` (拡張メッシュ Lx 0.40/R 0.20, 近傍解像度一致, 82k ノード, run_0073 から補間)。
  出口質量流束は理論値 0.143 kg/s の周りを周期 15000 step・±22 % で脈動 (旧領域と同じ周期)、内側 r≤0.1 と外側が同位相、
  x=0.25 面と出口面で同値、逆流なし → **coflow 柱全体の軸方向脈動で、領域の閉じ込めが原因ではない**。① (外周開放) 悪化、③ (precond 0) 振幅減のみ、
...
- `2026-09-05 (14)` — **P0-2 結論**: `run_0077` (dual-time dt 1e-5 s × 0.1 s) で出口流束が 0.037–0.038 kg/s (理論 0.037) に減衰収束、z/d 26 の
  10 ms あたり ΔT ≤2 K。擬似時間の周期 15k step・±20 % 脈動は物理には無い → 定常反復固有 (①外周開放は悪化、③precond 0 は振幅半減のみ、
  ②領域拡張は不変、④物理時間で消滅)。燃焼場は定常と dual-time で同一 (中心軸 T ≤15 K, z/d 26 半径 T ≤20 K 差)。運用: 探索は定常、報告は dual-time。
  実験との差 (z/d 9/11/14 の半径 T +222/+191/+153 K, 中心軸 z/d 20 +200 K, z/d 30 で一致) は数値・境界・機構では動かず TCI へ (`chemistry-cmc-tci.md`)。
- `2026-09-05 (14)` — `run_0079` (dual-time 100–200 ms): 内部場は完全定常 (z/d 26 ΔT ≤1 K)、出口質量流束は 0.0367 ± 0.002 kg/s (±5.5 %, 周期 ~20 ms)
  の小振幅物理振動で減衰が止まる (火炎域には無影響)。**Cabra 反応 ON (Li, tci 0) の定常参照場 = `run_0079/res_10000` (t=200 ms)**。
  P0-2 はここで閉じる (ツール・原因・運用が揃った)。残る P0 はユーザ判断の P0-1/P0-4 のみ。
