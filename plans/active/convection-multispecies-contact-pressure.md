...
理論・式は `methods/convection/theory.md` の追記節へ (本 plan には差分のみ)。要点:
...
1. **コード survey 文書化** (`methods/convection/implementation.md` に現行 face-state 生成フロー: gradient →
...
4. (Gate 1 のとき) **species face 2 次再構成 + positivity/simplex/fallback** 実装 (`convectiveFlux_d.cu`,
   config `speciesFaceReconstruction`/`faceLimiterCoupling`)。`methods/convection/{theory,implementation}.md` 先行更新。
...
- 触る: `cuda_forge/convectiveFlux_d.cu` (SLAU 経路), config (`speciesFaceReconstruction`,
  `faceLimiterCoupling`), 新規 1D ベンチ, `methods/convection/{theory,implementation}.md`, `methods/index.md`。
...
## 15. 「face Y は高次でなく中心補間で十分か」検証 (2026-06-20)

ユーザ仮説: S0→S2 改善の本質は *高次再構成* でなく *thermo を界面 Y で評価する* こと。
ならば thermo で MUSCL せず単純中心補間 `Y_f=f·Y_0+(1-f)·Y_1` (有界・非高次) を使えば S2 より安定で
同効果のはず → S2 は不要では、という主張を検証。中心補間版は既存の `FORGE_FACE_THERMOY` 経路
(`g_faceThermoY`) がそのまま該当。S0/S2c(中心)/S2(MUSCL) を同一 restart で比較 (移流は全て1次)。

### cfl=2 settled
| mode | rms_ro | A_P | A_ur | A_YHe | A_T |
| --- | --- | --- | --- | --- | --- |
| S0 (cell R_mix) | 1.68e-6 | 871 | 2.11 | 2.86e-3 | 1.04 |
| **S2c (中心 face Y)** | 1.15e-6 | 499 | 1.40 | 1.42e-3 | 0.52 |
| S2 (MUSCL face Y) | 9.49e-7 | 382 | 1.14 | 1.08e-3 | 0.28 |

### cfl=4
- S0 安定 / **S2c 発散 (step 638)** / S2 安定。

### 結論 (仮説は半分正しく半分逆)
- **正しい半分**: 「**界面 Y で thermo を評価する**」のが S0→S2 改善の主因。S2c (中心補間・非高次) だけで
  改善幅の **A_P 76% / rms 73%** を回収 = 高次再構成が効いているのではない、というユーザの読みは正しい。
- **逆だった半分**: 「高次をやめると S2 より安定」は**不成立**。S2c は cfl2 で S2 より僅かに悪く、
  **cfl4 で発散** (S2 は安定)。
- **理由 (3 実験を貫く統一原理)**: S2(MUSCL) は ρ と Y に**同一リミタ ψ_ρ** を掛けるので、sharp face で
  ψ_ρ→0 のとき ρ_L→cell0 **かつ** Y_L→cell0 と**揃って**1次化し ρ-Y 整合が保たれる。S2c は ρ だけ
  ψ_ρ で cell へ潰れ Y は中心ブレンドのまま → sharp face で **ρ は cell0・Y は両側ブレンド**の新たな
  ρ-Y 不整合が出て高 CFL で発散。これは min(ψ_ρ,ψ_Y) 発散・S3 発散と**同じ「ρ と Y は同一再構成で
  揃えねばならない」原理**の third instance。
- **含意**: 「界面 Y を使う」というユーザの本質理解は正しいが、それを**安定に**実装する最小形が
  まさに S2(MUSCL+共有 ψ_ρ) = "中心補間を ρ 整合にしたもの"。よって **S2 は不要ではなく、face-Y
  アイデアを高 CFL で安定化する最小の正しい形**。S2c は S2 に厳密劣位 (低 CFL でも僅悪・高 CFL で発散)
  なので production には採らない。S0/S2c/S2 比較 run: `run_0058..0063_thermoY_*`。
