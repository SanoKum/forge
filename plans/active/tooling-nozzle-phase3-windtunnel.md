...
- A2 **$\varepsilon_M$ を軸対称質量流束重みに修正**: 現行は $\rho u_x$ のみでメッシュ密度依存。**単純に $2\pi r\,dr$ を掛けるだけでは不十分** (半径方向が非一様) — 各セルに対応する**環状面積 $\pi(r_{out}^2-r_{in}^2)$** を使うか、**固定半径格子へ補間してから積分**する。測定面の docstring/実装不一致も解消し、**テストコア (有効菱形) の幾何定義を先に固定**する
