...
## 10. スロート直後の圧力波 — 訂正済みの作戦 (2026-08-14, Codex レビュー反映)

### 10.1 撤回・訂正した主張 (すべて私の誤り)

| 主張 | 判定 | 正しい理解 |
| --- | --- | --- |
| fill の向きを「修正」した | **撤回** | 楔を*誤ったデータ*で埋め、出口 ε_M が 0.173→0.932% に悪化。旧実装 (楔は空・凸包補間) を維持 |
| 曲率ブレンドで C2 を作る | **撤回** | 健全な設計データを 1.2 $r^*$ 上書きし更に悪化 (→1.166%)。既定 off、呼び出し側 fallback も 0.0 |
...
### 10.2 作戦 (この順で進める。A が終わるまで A/B の結論を出さない)

**A. 実験条件・測定の健全化**
- A1 blend を真に無効化 (呼び出し側 fallback 0.0 — 済) + 問題 YAML に明示。汚染 run (0004/0005 と初回 0006/0007) は台帳で破棄扱い
- A2 **$\varepsilon_M$ を軸対称質量流束重みに修正**: 重み $\rho u_x\,2\pi r\,dr$ (現行は $\rho u_x$ のみでメッシュ密度依存)。測定面の docstring/実装不一致も解消し、テストコアは**有効菱形**で定義
- A3 **各パスに CFD 健全性ゲート**: DIVERGED 除外 + quasisteady + 残差の**絶対レベル**を記録 (warm restart では「低下桁数」判定は無意味)。ゲートを通らない場を「収束場」と呼ばない

**B. C2 接続を保証する (本丸)**
- B1 **目標分布の始端 C2 化** (Sivells の古典要件 — サーベイ A0/B2): $M''(x_0)$ を Sauer に整合させる。現行は符号違いで**波源の第一容疑**。CP を 1 個増やし探索次元は不変に保つ
- B2 **下流壁を拘束付き表現へ** (ユーザ提案 + Codex 提案): 始端で $r,r',r''$ を円弧に一致させた B-spline を逆 MOC 点列に**最小二乗フィット**。バッファ領域を作らず、形状表現そのものに C2 を内蔵する
- B3 B1/B2 の効果を、収束ゲート付き Euler A/B で「壁曲率の跳び」と「圧力波」の消失として確認
...
**D. 記録の同期** — plan とコード docstring を巻き戻し後の状態に合わせる (本節で plan は完了、コード側は A と同時に)
...
- `2026-08-14` — **スロート直後圧縮波の根本原因調査** (ユーザ指摘「二階微分がスムーズでない/
  ...
  1. **誤診の訂正**: 「目標側 $M''$ を合わせても壁曲率に効かない」は誤診だった。真因 (第一) =
     ...
     → 両割当てを試し前方交点を採る向き非依存 fill に修正。単体 ALL PASS 維持、
  ...
  2. **第二の構造限界**: 修正後も壁足曲率は +0.03 で円弧の 0.51 に届かない。レベル充填は
     ...
     戻して ALL PASS 状態で退避)。
  3. **暫定対策**: `ModeFWall` に円弧→設計壁の**曲率ブレンド (5 次 Hermite, C2)** を実装
     (収縮→円弧接続と同じ手法)。blend_len スイープ: 0.4 で振れ幅 −0.47、1.2 で −0.20
     (完全解消は C⁺ マーチ完成後に幾何側ブレンド不要化を再判定)。
