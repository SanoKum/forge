...
**B0. 遷音速 starting line の壁角整合 (2026-08-14 実測で発見・実施済み)**
- Sauer 解は**線形化壁 BC** $\bar v = dr_w/dx$ を満たすよう構成されているのに、
  `theta()` が $\arctan(\bar v/(1+\bar u))$ と完全な $u$ で割っていたため、**壁角が
  25.7% 過小** (R=2 で 7.394°→5.892°) だった。starting line の時点で円弧と **1.5° の
  キンク**を作る = 圧縮波の源。
- `theta_mode="linearized"` ($\theta=\arctan\bar v$) を既定化して**不整合 1.502°→0.244°**
  (6 倍改善)、リップ半径 / 1D√ε も **0.9964→0.9990** に改善。両スイート ALL PASS。
- **根治は Kliegel–Levine 高次**: 「① は $R\gtrsim1$ なので Sauer で足りる」という §2 の
  判断は**この実測で反証された**。残差 0.244° と壁足の扱いを詰めるには KL が要る。
