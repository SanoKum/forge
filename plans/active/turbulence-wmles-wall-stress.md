...
- `2026-07-20` — **壁ノード密度ドレインバグを発見 (チャネルの現律速)**: 等温ピン (T/roe/P/sonic
  上書き + 連続式は自由) の下で、乱流場では壁ノードの ρ が場 (1.185 保存・健全) から乖離して
  0.48 まで排出され P_wall→42kPa。壁モデルは p_w=P[壁ノード] から ρ_w を組むため
  **τ_w=ρ_w u_τ² が目標の 39% に半減** (u_τ 速度スケール自体は −1.6% で正常)。純伝導 (case/24)
  では発生せず (P 一様保持) — 乱流変動 + ES 散逸 + ピン状態の W-I 大ジャンプの組合せで駆動される
  疑い。**次タスク: ドレイン機構の特定 (W-I 面の質量流束成分の切り分け) と対策** (候補: ES 散逸の
  質量成分をピン壁隣接面で除外 / 壁ノード ρ の整合ピン)。
