...
- `2026-07-20` — **§5-8 チャネル検証開始 (case/38 新設)**。統合スモーク合格 (WMLES 稼働・壁 bvar
  整合)・乱流自己維持確認。**ベースライン (KEEP+ES σ0.05 + venkata limiter + WALE) は静定後判定で
  不合格**: u_τ 3.72±0.08 で 150 FT 静定するも、大域運動量 −43%・log 層 u⁺ +42%・第 1-2 内点に
  非物理ジャンプ (I1-I2 面の乱流応力欠損を分子粘性勾配が肩代わりする擬似平衡)。壁モデル自体は
  健全。較正 A/B (実質 σ0.05→0.02 単変数; limiter は KEEP スタック不関与) は静定後評価で
  **ジャンプ Δu⁺ 14.7→10.4・log 層 +42→+30% と実効**。教訓: u_τ プラトー前の診断は禁止
  (途中経過を早診断してユーザー指摘で撤回した経緯を含め case README 参照)。
- `2026-07-20` — **⚠撤回: 「壁ノード密度ドレインバグ」は誤診断だった**。場の壁ノードは健全
  (ro=1.217・P=104.7kPa・T=300.0 厳密) で、ρ_w=0.48 は**壁 bvar 出力 (res_<wall>_*.h5) の
  ro/Psb が field と別位相で書かれる出力アーティファクト** (これは実在の小バグとして残タスク化:
  壁 bvar 出力の位相整合)。真の応力配分は壁モデル経由でほぼ全量 (τ_w≈17.5≈fδ) — 釣合いは閉じて
  いる (ユーザー指摘 2 回目で確定)。**チャネルの残る唯一の問題は log 層 +30% の近壁混合不足**:
  WALE/σ/Vreman はいずれも「純せん断で νt→0」の設計のため WMLES 第 1-2 ノード帯で νt が
  必要量 (κyρu_τ) の ~1/6。対策 2 トラック: (A) WMLES 壁影響層の mixing-length 床
