...
- **W4 cancellation MOC の仕様 (2026-08-15 レビュー P1 反映 — 着手前確定分)**:
  ...
  - **非反射 (cancellation) 条件**: 壁点で追加の compatibility は**課さない**。壁は
    「データ線+軸だけで決まる場の流線」であり、D から $dr/dx=\tan\theta$ (場の線形補間)
    を積分して各 C⁺ フロントとの交点を壁点とする (`moc_inverse` の壁抽出と同一機構)。
    壁が場の流線に一致することが反射 C⁻ 強度 0 と等価 (Zucrow–Hoffman)。壁流線が
    充填域の外へ出た場合はエラー (データ線の下流延長不足のサイン)。
  - **終端条件**: 壁角 $|\theta_w|\le\theta_{tol}$ (初期値 0.01°) かつ軸 $M$ が
    $M_d$ の要求帯 ($\varepsilon_M$ 相当) に入った C⁺ フロントで打ち切り、以降は円筒。
  - **$P_0$ 診断の合否 (初期値 — W4 実装時の照合で再調整可)**: データ線上の
    $|\Delta P_0|/P_0 \le 0.1\%$ = PASS (非回転 MOC 続行) / 0.1–0.5% = WARN (続行、
    報告に併記) / $>0.5\%$ = FAIL (回転 MOC 実装または CFD 分解能改善まで W5 に進まない)。
  - **照合 (W4 の受け入れ条件)**: (i) 放射源流厳密解のデータ線から場を <0.1% で再現
    (`moc_kernel` の既存照合と同一基準)、(ii) データ線・充填格子の 2 倍細分で壁座標変化
    <0.5% (格子収束)、(iii) 自己一貫: B8 逆 MOC 場から同位置の C⁻ を抜いてデータ線に
    与え、生成壁が B8 設計壁と $\varepsilon_M$ 相当以内で一致すること。
