...
| [tooling-nozzle-axismach-chain.md](active/tooling-nozzle-axismach-chain.md) | `tooling / optimization` | ①風洞の **axis-Mach チェーン** (2026-08-15 起票、主線): Hall 遷音速 + **5次 Hermite 軸 Mach law (自由度 $L_c$ のみ)** + 逆 MOC + CFD-in-the-loop アンカー更新 ($x_A=x_{\rm reach,CFD}$, 平滑化フィットで $M_A,M'_A,M''_A$)。B10-c (3 自由 CP で両端 C² 両立不能) を Hermite で構造的に解消、B6 (CFD アンカー有効) を反復化。E/F 区別と $L_c$ 長さ制御を新設。上流壁 = **入口直管 + U→T 5次 Hermite** (W1 資産流用、上流円弧廃止)。B8 と同条件比較で判定 |
