...
- **先例 (重要)**: 軸対称 MARKER_SYM で「残差射影だけでは implicit/block-DPLUR に非整合 (Mach~1000 発散)、in-Jacobian で roUy 行 (index2) を decouple すれば整合」が確立済み ([docs/discretization/implementation.md](../../docs/discretization/implementation.md) の SU2 比較節)。本 plan は同じ機構を**壁の運動量3行**へ適用する。
...
- 理論変更は軽微 (既存 SU2 比較節に壁を追記) のため、実装前に docs/discretization/{theory,implementation}.md に壁 in-Jacobian decouple の小節を追記する。
...
- ドキュメント: `docs/discretization/theory.md` §6.3 (壁 Dirichlet の implicit 整合)、`docs/discretization/implementation.md` (SU2 比較節に壁 in-Jacobian decouple を追記)、`docs/index.md` の整合確認。
