...
| `sern_2d` | `evaluate/runner_sern.py` (予定) | 🔧 | ⑤ SERN: 燃焼器出口 starting line + 2D 平面最大推力 key point 逆設計 (`geometry/moc_sern.py`) + メッシュ/評価 (`meshing/mesh_sern.py`, `evaluate/runner_sern.py`, `metrics/sern_forces.py`)。**cell Euler で MOC・NASA TM X-71972 傾向と照合済み** (case/46 run_0002–0007)。RANS 化・多作動点 MOO は未。plan: [`plans/active/tooling-nozzle-sern-chain.md`](../plans/active/tooling-nozzle-sern-chain.md) (2026-09-04 起票) |
