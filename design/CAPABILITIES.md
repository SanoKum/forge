...
| `sern_2d` | `evaluate/runner_sern.py` | ✅ | ⑤ SERN: 燃焼器出口 starting line + 2D 平面最大推力 key point 逆設計 (`geometry/moc_sern.py`) + メッシュ/評価 (`meshing/mesh_sern.py`, `evaluate/runner_sern.py`, `metrics/sern_forces.py`)。**cell Euler で MOC・NASA TM X-71972 傾向と照合済み** (case/46 run_0002–0007)。RANS は cell+SST (node+SST は発散・未解決)、δ* 一発補正 (`metrics/sern_deltastar.py`)、多作動点 MOO (`opt/driver_sern.py`, case/46 run_0010: Euler 2 作動点 17 PASS) まで検証済み。plan: [`plans/active/tooling-nozzle-sern-chain.md`](../plans/active/tooling-nozzle-sern-chain.md) (2026-09-04 起票) |
