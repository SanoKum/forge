...
- `08.bump`: line profile 比較を含む既存の回帰確認フローが必要なときに使う。
...
- 検証を実行するときは、既存の `run_*` を使い回さず、必ず複製した新しい `run_*` ディレクトリで実行する。
- 生成される `residual_history.csv`、`res_*.h5`、壁面出力、実行ログは、その新しい `run_*` に保存して比較可能な状態を維持する。
- 検証 run では、`residual_history.csv` の生成で止めず、同じ `run_*` に `residual_history.png` も出力する。
