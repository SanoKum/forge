...
- `2026-06-10` — **Track B: Cutler 同軸ジェット発達場の clean 化 (M7 が遠方場 entrainment 病理を解消) + 定量照合ツール整備**。
  ...
  - **残**: 実験 (RELIEF survey) の He モル分率・pitot・全温プロファイルのデジタイズ (`cutler_exp_centerline.csv` 等) が無いため、定量オーバーレイ照合は未。次は ① 実験データ投入で定量照合、② 3 成分 [He,O2,N2] を同様に M7 で healing → 発達 (`run_0022` openbc 系)。
