...
- **多成分 implicit 結合の恒久修正 (要因2)**: `physProp.thermoHrefTemp` (sensible-enthalpy datum, 2026-06-19) で安定 `cfl_pseudo` 上限を 1→2 に上げた (部分緩和)。続けて **案B (緩和整合 scalar-DPLUR, `speciesImplicitCoupling`)** を実装・検証 → **無効と判明** ([thermophysics-species-implicit-coupling.md](thermophysics-species-implicit-coupling.md), case/16 README run_0110〜0120)。理由: ① matched≡off で cfl 上限が一切動かない、② 単成分 N2 も同 cfl4 で発散=ボトルネックは**流れ/SST block-DPLUR 律速**で化学種は最早ボトルネックでない、③ 組成完全一様で `res_roY≈0`=移流緩和は無関係 (H2O ペナルティは EOS エネルギー再構成側)。**次の高優先は流れ/SST 陰解法の cfl 上限引き上げ** (`time_integration-implicit-stable-cfl.md` 参照)。要因2 残ペナルティ (cfl3 で 2sp<1sp) の恒久修正には EOS cross-Jacobian (案A: species を 5×5+N の単一 block に同梱) が必要だが、流れ/SST 上限が先に律速するため優先度低。
