...
- **多成分 implicit 結合の恒久修正 (要因2)**: roe(block-DPLUR) と roY(別 point-implicit) の緩和ミスマッチが残る。`physProp.thermoHrefTemp` (sensible-enthalpy datum, 2026-06-19) で生成エンタルピー増幅を除き安定 `cfl_pseudo` 上限を 1→2 に上げたが**部分緩和に留まる**。厳密収束・高 cfl には species を 5×5 (+nSpecies) の単一 block に同梱した full-coupling Jacobian、または roe/roY の緩和係数を整合させる実装が必要。
