...
- `2026-06-10` — **Track B: Cutler 同軸ジェット発達場の clean 化 (M7 が遠方場 entrainment 病理を解消) + 定量照合ツール整備**。
  ...
  - **残**: 実験 (RELIEF survey) の He モル分率・pitot・全温プロファイルのデジタイズ (`cutler_exp_centerline.csv` 等) が無いため、定量オーバーレイ照合のみ未。binary/ternary とも物理的に妥当な clean 収束場は得られており、実験データ投入で即照合可能。
