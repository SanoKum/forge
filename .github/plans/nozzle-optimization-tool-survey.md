...
- **(b) 回数固定の NS 壁更新（infill 段の推奨）**: forge 解から δ\*(x) を測り、壁を 1 回（最大 2 回）だけ更新して再評価。1 設計点 = forge 2–3 回。同一設計点内で閉じた更新なので決定性は保たれる（「lagged 補正の禁止」は前の設計点の δ\* 使い回しの話で、これは別物）。**回数を固定する**ことが肝 — 許容誤差ベースの打ち切りは「設計変数の微小変化で反復回数が変わり形状が跳ぶ」不連続を写像に持ち込み、Kriging が苦しむ。回数固定なら写像は決定的かつ滑らかでコスト上限も明確。副産物: 1 回目の解は co-kriging の低忠実度層に流用できる（B3）。
