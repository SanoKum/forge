...
- **③ SERN**: 使える — ただし**参照線を「中心線」から「ランプ壁」に変える**のが得策。対称軸が無いので、**ランプ壁の目標圧力（またはマッハ）分布 $p_w(x)$ の Bézier を設計変数**にする（2D 平面 MOC の逆設計は壁分布指定でも成立）。SERN では推力・揚力・ピッチモーメントが**ランプ圧力分布の積分そのもの**なので、フロー空間変数と目的関数の結び付きは中心線方式以上に直接的【○】。
  ...
  - **注意点**: (i) 波キャンセルは設計 NPR 1 点でのみ成立 → 多高度オフデザインは forge 評価シナリオ側で持つ（B4）。(ii) スロートが非対称なため Hall/Kliegel–Levine（対称仮定）の starting line がそのまま使えない → 内部ノズル部を僅かに超音速（M≈1.1–1.2）まで対称的に設計してから ramp 膨張に接続するか、遷音速部は forge 解から starting line を取る。(iii) 有限スパン・サイドフェンス・角部 R の 3D 効果は 2D MOC の外 → FFD（A3）で重ね、3D forge RANS で評価（A9 の文献空白＝新規性領域）。
