...
- **starting line の取り方**: M=1 ちょうどの線は特性線が退化（マッハ角 90°）して初期値線に不適。**コア全域で M ≳ 1.05–1.2 となる断面（spacelike）**を forge 解から取り、範囲は**軸〜境界層端のみ**（BL 亜音速部は δ\* 側で扱う）。NS 由来の線はエントロピー非一様を含むため**回転流 MOC**（エントロピー勾配ソース項付き、Zucrow & Hoffman の標準拡張）を使う。
