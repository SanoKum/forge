...
- **(B) 特性線トレース帰還（推奨）**: forge の **NS 場そのものの中で**、壁（BL 端）から特性線を局所マッハ数に従い数値積分でトレース（$dy/dx=\tan(\theta\mp\mu)$ を場の補間で積分）し、軸到達点での達成マッハと目標マッハの残差を、その特性線の根元の**壁角補正**に翻訳（Prandtl–Meyer 不変量: $\Delta\theta_w \approx \Delta\nu$、緩和係数 0.3–0.5）→ 壁再スプライン → 再計算（warm restart）→ 反復。
