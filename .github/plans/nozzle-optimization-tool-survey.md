...
1. **設計変数**: 目標中心線マッハ分布 $M_c(x)$ の Bézier 制御点（5–8 個。**単調な制御多角形 → 単調な分布**が保証される）＋上流物理量（スロート径（スケール/流量）, $R_c$, 収縮パラメータ; A2.5(E)）。
2. **形状生成器（≒既存資産の移植/ラップ）**: 遷音速 starting line（Kliegel–Levine, $R_c$）→ **既存 MOC/2D Euler 逆設計**で $M_c(x)$ を実現する非粘性コンタ → **決定的な積分法 δ\*(x) 補正**を加算（①は必須級・②③は任意。方針は後述「境界層 (δ\*) 補正の方針」）。
...
### 「目標達成」をどこまで強制するか（3 段階の使い分け）

- **(a) パラメータ化として割り切る（DOE・広域探索の既定）**: 1 設計点 = forge 1 回（＋決定的 δ\* 積分補正）。達成分布と目標のズレは最適化器が吸収。安価・単純。
- **(b) 回数固定の NS 壁更新（infill 段の推奨）**: forge 解に基づき壁を 1 回（最大 2 回）だけ更新して再評価（更新則は δ\* 一致または**特性線トレース帰還** — 後述「壁更新エンジンの 2 実装」の (B) を推奨）。1 設計点 = forge 2–3 回。同一設計点内で閉じた更新なので決定性は保たれる（「lagged 補正の禁止」は前の設計点の δ\* 使い回しの話で、これは別物）。**回数を固定する**ことが肝 — 許容誤差ベースの打ち切りは「設計変数の微小変化で反復回数が変わり形状が跳ぶ」不連続を写像に持ち込み、Kriging が苦しむ。回数固定なら写像は決定的かつ滑らかでコスト上限も明確。副産物: 1 回目の解は co-kriging の低忠実度層に流用できる（B3）。
- **(c) 完全収束の逆設計内ループ（Korte 流・最終 polish 限定）**: パレート採用点に対してのみ「forge 解 → δ\* → 壁更新 → forge」を収束まで反復し、目標分布を粘性込みで達成させる（CAN-DO の最小二乗マッチングの現代版, A2.5(D)）。全設計点で行うとコスト 3–5 倍になるうえ、BL 端検出ノイズで内ループがリミットサイクルに陥るリスクがある。

> **壁更新の一致条件の注意 — 「境界層外縁 = Euler 壁」は過補正**: 変位物体の理論では、外側非粘性流が設計 Euler 場に一致する条件は「**物理壁 − δ\*（排除厚さ）＝ Euler 設計壁**」。境界層**外縁**（δ）を Euler 壁に一致させると常に δ − δ\* だけ壁を出しすぎ（δ\* < δ。極超音速では δ\*/δ は大きいが 1 未満）、有効流路が広がって**出口マッハが目標より高く収束**する。一致対象は δ\* 分の有効壁とし、δ\* 算出の BL 端定義は「断面最大速度の 95%」が設計マッハ回復に最良という CFD ベース補正の知見（A2, 2014 韓国論文）に従う。

### 境界層 (δ\*) 補正の方針 — 壁は動かす。ただし「決定的」に

> 「逆設計コンタに δ\* 見積りを加えて壁を動かしてから forge に渡すべきか？ 見積り誤差が最適化を汚染しないか？」への回答。
> **結論: ① は動かす（積分法の決定的 δ\* 補正を形状生成器に内蔵）、②③ は無補正で開始してよい。いずれの場合も「設計変数のみの決定的な関数」であることを死守する。**

1. **δ\* の誤差は最適化にバイアスを与えない**。目的関数は常に「実際に評価した壁形状」に対する forge 粘性解から測るため、δ\* 見積りがどれだけ粗くても、得られるパレートは実物理上で正しい。δ\* 補正は「設計変数 → 形状」写像の一部に過ぎず、その誤差が効くのは**パラメータ化の中心が良い設計群にどれだけ揃うか（探索効率）だけ**。
2. **ただし写像の決定性が壊れると最適化が壊れる**。同じ設計変数 $x$ が評価のたびに違う形状を生む（＝目的関数が履歴依存になる）と、Kriging サロゲートは「同一入力・異出力」をノイズとして処理せざるを得ず、infill 判断が劣化する。**禁止事項: 直前の forge 解から取った δ\*(x) をループ内で逐次使い回す lagged 補正**（評価履歴に依存する写像になる）。
3. **推奨実装**: 積分法（von Kármán 運動量積分や平板相関）で MOC 端条件から δ\*(x) を解く**決定的・安価（≪forge 1 回）な補正**を形状生成器に内蔵する（＝Sivells の排除厚さ補正を生成器の一部として温存する形）。誤差ゼロは目指さない — 中心合わせができれば十分で、残差は最適化器が吸収する。
   - **① 極超音速風洞では必須級**: δ\* が半径に対し一次のオーダーになり、無補正では「単調 Bézier 分布 × 非粘性逆設計」の形状族が良設計群から系統的に外れる（古典実務が常に補正してきた理由）。
   - **②③ は無補正開始で可**: BL が相対的に薄く形状族の中心ズレは小さい。必要になってから同じ機構を有効化する。
4. **再較正はラウンド単位で**: 蓄積した forge 解から積分法の係数（形状係数・遷移位置など）を更新したくなったら、**DOE/サロゲート再構築の節目でのみ**写像を更新し、旧評価点は破棄するか別忠実度として扱う（co-kriging の低忠実度層に降格）。ループ途中の写像変更は 2. と同じ理由で不可。
5. **NS 解に基づく壁更新は段階に応じて**: infill 段は回数固定 1 回（上記 (b)）、収束までの完全反復（上記 (c) Korte 流）はパレート採用点の最終 polish 限定。一致条件は境界層外縁でなく δ\* の有効壁（前節の注意参照）。

### 発展形 — NS 由来 starting line のハイブリッド（遷音速は forge、超音速設計は MOC のまま）

> 「粘性計算しながら M=1 近傍の断面を特定し、そこから特性線を飛ばす」方式。解析遷音速解（Hall/Kliegel–Levine）の仮定（完全ガス・対称スロート・壁曲率一定・非粘性）を全部外せる。**CAN-DO のスロート NS＋超音速 marching の領域分割（A2.5(D)）と同じ哲学**であり、特に**小 $R_c$（級数適用域外）・SERN 非対称スロート・多成分 TP/壁熱伝達でソニックラインがずれる場合**に有効。

- **starting line の取り方**: M=1 ちょうどの線は特性線が退化（マッハ角 90°）して初期値線に不適。**コア全域で M ≳ 1.05–1.2 となる最初の断面（spacelike）**を決定的ルールで選び、forge 解から**軸〜境界層端のみ**のデータ（マッハ・流れ角・エントロピー＋コア質量流量・BL 端位置）を取る（BL 亜音速部は δ\* 側で扱う）。NS 由来の線はエントロピー非一様を含むため**回転流 MOC**（エントロピー勾配ソース項付き、Zucrow & Hoffman の標準拡張）を使う。
- **概念整理 — MOC は「壁から」飛ばさない・逆設計では壁は出力**: 直接法・逆設計いずれも MOC の出発点は壁上の点（ソニック点）ではなく**流れを横断する初期値線**（古典 CONTUR でも遷音速解が与えるのは横断線データ）。直接法では壁点は行進中に接線条件で作られ、**逆設計では壁はそもそも入力でなく、IVL の外端（BL 端）を通る境界流線として「求まる」**（流線ゆえ接線条件は自動、質量保存が一意化）。したがって「壁を動かしたらどこから特性線を飛ばすか」という問いは逆設計では生じない — x0 下流の壁は MOC を飛ばす時点で存在しなくてよい（それが出力だから）。壁面近傍でソニックラインが BL 端に触れる点は x0 上流の固定領域（NS が解く遷音速パッチ内）に属し、MOC は触らない。
- **壁更新の基準（x0 = starting line 断面で切り分け）**:
  - **x0 上流（収縮・スロート）は内ループで動かさない** — 設計変数（収縮パラメータ・$R_c$・スロート径）が直接決める領域で、外側最適化器が探索。その良否は starting line の質として forge が評価する。
  - **x0 下流: 物理壁 = 回転 MOC 逆設計の有効コンタ（変位物体）＋ δ\*(x)**。δ\* は **x0 での NS 実測 BL 状態（$\theta$, $H$）を初期値**に、MOC 端条件下で積分法を下流へ積分（決定的・安価）。一致条件は前節の通り δ\*（外縁ではない）。
- **収束が速い構造的理由**: 下流は超音速なので壁の再設計は上流へほぼ伝播しない（BL 亜音速部経由の弱経路のみ。剥離が無ければ無視可）→ **starting line はほぼ不変量**で、内反復は 1–2 回で足りる（回数固定方針と整合）。コスト: 1 設計点 = forge（上流＋仮下流）1 回 ＋ MOC ＋ forge（全域検証）1 回 ≈ 上記 (b) 段相当。
- **「壁を動かすと starting line も変わるのでは」への整理**: 厳密には変わるが、3 層で成立する。(i) **物理** — 変化は二次的（上流影響は BL 亜音速部の O(δ) のみ、x0 の BL 状態は上流履歴で決まる）。(ii) **アルゴリズム** — 内ループは（壁, starting line, δ\*）の不動点反復であり、starting line は不変である必要はなく「ほぼ不変」なら縮小写像として速く収束する。次の forge 全域解で取り直せばズレは次反復が吸収。(iii) **最適化の正しさ** — 回数固定で打ち切って不整合が残っても、目的関数は最終形状の forge 全域解から測るため「達成分布の目標からの微小なずれ」になるだけで、外側の最適化器が吸収する。**壊れるケース**: スロート近傍の剥離・遷音速衝撃では亜音速遡上が強まり内ループが振動し得る → 剥離チェック（制約）と壁更新の緩和係数で防ぐ。
- **位置づけ**: (a)(b)(c) の解析 starting line（Kliegel–Levine）を NS 由来線に置き換える上位互換オプション。①の通常域では解析線で十分なので、まず解析線で立ち上げ、小 $R_c$ 探索域・③ SERN でこのハイブリッドに切り替えるのが実装順として合理的。

### 壁更新エンジンの 2 実装 — オフライン逆設計 (A) と NS 場の特性線トレース帰還 (B・推奨)

(b)(c) 段の「NS 解に基づく壁更新」の実装は 2 通りある。

- **(A) オフライン逆設計**（前節まで）: forge 解から IVL（初期値線 = MOC が未知場の構築を始めるための、流れを横断する既知データ線）を抽出 → **CFD しない回転流 MOC** が下流の壁を逆設計 → forge で全域再検証。利点: MOC が一発で大域的な壁を出す。負担: 回転流 MOC 逆設計コード（最大の新規開発）と δ\* 推定が要る。
- **(B) 特性線トレース帰還（推奨）**: forge の **NS 場そのものの中で**特性線を数値積分でトレース（$dy/dx=\tan(\theta\mp\mu)$ を場の補間で積分）し、軸上の達成マッハと目標マッハの残差を、その特性線の根元の**壁角補正**に翻訳（Prandtl–Meyer 不変量: $\Delta\theta_w \approx \Delta\nu$、緩和係数 0.3–0.5）→ 壁再スプライン → 再計算（warm restart）→ 反復。
  - **起点は物理壁でなく境界層端**: 壁面は no-slip で M=0 のため特性線は出せないが、超音速コアの膨張波を発生させているのは**BL 端の流線（有効壁）**なので、トレースが BL 端で止まるのは欠陥でなく「波の発生源まで遡れている」ということ。物理壁を局所的に $\Delta\theta$ 回すと BL はその上に乗ったまま回り、**BL 端流線もほぼ同じ $\Delta\theta$ 回る**（$\theta_{edge}\approx\theta_{wall}+d\delta/dx$、BL 厚み応答は二次的）→ 挺子は境界層を素通しで伝わり、残る誤差は更新 Jacobian の誤差（緩和が吸収、収束結果にバイアスなし）。
  - **実装は軸から逆向きトレースが楽**: 軸上の残差点から C+ 特性線を場の中で上流へ逆積分し、**BL 端に当たった位置の真下の物理壁点**に $\Delta\theta_w$ を割り当てる。「どの壁点から飛ばすか」の選定問題が消え、wave-tracing 補修実務と同じ向きになる。
  - **補正は点ごとに当てず基底へ射影**: BL 端検出（95% $u_{max}$ 等）のノイズが壁角に乗るため、$\Delta\theta_w(x)$ をスプライン/Bézier の壁基底に射影して滑らかな成分だけ適用する。
  - **IVL も逆設計 MOC も不要**: 場が既知なので特性線は「なぞるだけ」。どこからでも引ける。
  - **δ\* 推定が消滅**: 不動点の定義が「粘性解の中心線分布 = 目標」なので、境界層変位は壁角に自動で織り込まれる（δ\* と外縁のどちらを合わせるかという問題自体が無くなる）。
  - **補正則の誤差はバイアスにならない**: PM 換算は更新方向の近似 Jacobian に過ぎず、収束判定は実 NS 場で行う → 収束すれば結果は厳密（近似が粗いと収束が遅いだけ）。
  - **系譜**: 風洞ノズル補修実務の wave-tracing 補正（試験部擾乱を特性線で壁位置まで遡り局所再加工）と同系【△ 要追検証、Shope 2006 参照】。
  - **注意**: 軸対称では波が軸向きに増幅され $\Delta\theta\to\Delta M$ が 1:1 からずれる（緩和で吸収）／反射波の結合は反復が吸収／壁面 M ≲ 1.1 の遷音速パッチは補正対象外（挺子が効かず、そこは設計変数の領域）／ループ内は回数固定 1–2 パス、完全収束は polish 限定（既定方針の通り）。
  - **コスト**: 1 パス = forge 1 回（warm restart で増分小、GPU 向き）。
- **推奨分担**: **(B) を壁更新エンジンの主実装**に（開発量: 特性線トレース＋PM 換算＋緩和のみ）、**(A) 系の MOC は初期形状生成器（既存資産＋解析 starting line）として温存**。これにより新規開発の最大物だった「回転流 MOC 逆設計」はオプション扱いに格下げできる。

### 機種適用

- **① 風洞**: 本方式が主軸（Sivells 系の直系。CONTUR の多項式中心線分布の Bézier 一般化）。
- **② DACS**: 軸対称コンタ部に適用可（推力目的でも生成則としては機能する）。スカーフ角・長さは形状側変数として併用。
- **③ SERN**: 使える — ただし**参照線を「中心線」から「ランプ壁」に変える**のが得策。対称軸が無いので、**ランプ壁の目標圧力（またはマッハ）分布 $p_w(x)$ の Bézier を設計変数**にする（2D 平面 MOC の逆設計は壁分布指定でも成立）。SERN では推力・揚力・ピッチモーメントが**ランプ圧力分布の積分そのもの**なので、フロー空間変数と目的関数の結び付きは中心線方式以上に直接的【○】。
  - **MOC 側の扱い**: カウルリップ起点の中心膨張扇＋ランプでの反射波キャンセル（最小長ノズルの片側版が SERN の古典設計）。カウル後縁以降の下側は自由境界（$p=p_\infty$）条件で扱える（Zucrow & Hoffman の標準手法）【○】。
  - **注意点**: (i) 波キャンセルは設計 NPR 1 点でのみ成立 → 多高度オフデザインは forge 評価シナリオ側で持つ（B4）。(ii) スロートが非対称なため Hall/Kliegel–Levine（対称仮定）の starting line がそのまま使えない → 内部ノズル部を僅かに超音速（M≈1.1–1.2）まで対称的に設計してから ramp 膨張に接続するか、**NS 由来 starting line のハイブリッド（前節）**を使う。(iii) 有限スパン・サイドフェンス・角部 R の 3D 効果は 2D MOC の外 → FFD（A3）で重ね、3D forge RANS で評価（A9 の文献空白＝新規性領域）。
...
| ★★★ | MOC 逆設計のループ内化 | 既存 MOC/2D Euler 逆設計コードを「目標中心線マッハ分布 Bézier → 非粘性コンタ」の呼び出し可能な形状生成器としてラップ（B2.5）。Bézier 制約チェック（単調性・スロート整合・膨張率上限＝凝縮事前フィルタ）を同梱。①向けに決定的な積分法 δ\* 補正を内蔵（lagged 補正はループ内禁止 — B2.5） | 中 |
| ★★★ | 特性線トレース帰還エンジン | forge 解の中で壁（BL 端）→軸の特性線を数値トレースし、軸上マッハ残差を壁角補正に翻訳（B2.5 壁更新エンジン (B)）。緩和係数・回数固定・遷音速パッチ除外を含む | 中 |
