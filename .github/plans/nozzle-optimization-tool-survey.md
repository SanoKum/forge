...
| 条件 | 内容 | 根拠 |
| --- | --- | --- |
| スロート整合 | モード F: $M_c(x^*)=1$・$dM_c/dx$ を遷音速解（$R_c$）と整合／モード K: $x_k$ で場の $M, M'$ を継承（下記） | starting line／kernel 接合（A2.5(A)(B)） |
...
- **モード K（kernel 承継・推奨）**: 下流円弧（$R_d, \theta_a$）が**円弧終端 $P_a$ までの壁を所有**し、帰還エンジンは $P_a$ より下流だけを補正する。kernel 域（円弧が支配する初期膨張）の中心線マッハは $(R_c, R_d, \theta_a)$ の**結果**であり、目標 Bézier は **kernel 境界 $x_k$**（$P_a$ からの最終特性線が軸に当たる点 — 場内トレースで決定）**より下流のみ**を規定する。Bézier 左端の値・勾配は場の $M(x_k), M'(x_k)$ を継承（C1 接続）し、残りの制御点が自由変数。Rao（円弧→変曲点→キャンセル域）・Sivells（遷音速整合）と同じ階層構造で、$\theta_a$ が「初期膨張の速さ＝短さ vs 一様性・熱流束」の独立した設計変数として効く。
- **モード F（分布全権）**: $\theta_a$ は遷音速パッチを抜ける最小固定値とし、Bézier がスロート直後から全域を規定（壁は帰還エンジン任せ）。$\theta_a$ は設計変数から消える。単純で変数が減るが、初期膨張率の制御が Bézier 左端勾配に間接化する。
...
| ★★★ | 目的関数ライブラリ | `res_*.h5` から推力 $C_F$・軸長・出口マッハ一様性（コア径）・熱流束・剥離指標・$C_d$ を算出（既存 [validate_*.py]/probe を一般化） | 中 |
...
10. **【新規 2026-06-13】機種改訂に伴う未調査領域**: (a) ④デュアルベル — Frey & Hagemann, "Critical Assessment of Dual-Bell Nozzles," *J. Propulsion and Power*, 15(1):137–143, 1999（CP/PP エクステンション分類・遷移条件）ほか DLR 系実験（Stark/Génin）の一次確認、遷移ヒステリシス・横荷重の RANS 妥当性。(b) ②矩形風洞 — 側壁 δ\* を上下コンタへ転嫁する古典手順（CONTUR planar の側壁扱い）の一次確認、角部流の影響定量。(c) ⑤ 3D in-loop の FFD 変数数とサロゲートの実用限界（B4 随伴再考トリガの定量化）。(d) wave-tracing 帰還（B3）の一次文献固め（Shope 2006 ほか風洞補修実務）。
