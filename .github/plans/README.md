...
| [thermophysics-multicomponent-tpgas.md](thermophysics-multicomponent-tpgas.md) | thermophysics | in_progress | [`docs/thermophysics/`](../../docs/thermophysics/) | 多成分 thermally-perfect gas (非反応)。`thermalMethod==2`: NASA-9 比熱 (CEA 準拠)・Newton 温度反転・ideal-gas mixing・kinetic theory 輸送係数。**M1 (単成分 NASA-9 + SLAU/ROE の TP 整合 + 熱力学コア) 実装・native ビルド成功・NASA-9 単体値を NIST と照合済 (N2: R=296.8, cp(300)=1040, cp(2000)=1284, h(298)=0, e→T 反転 ~1e-11)・CPG 経路の回帰不変を確認**。流れ場検証は TP 整合な境界条件 (inlet/wall/outflow の roe/P を NASA 基準で再構成) が前提と判明し実装中。M2 (多成分輸送)・M3 (kinetic 輸送係数)・M4 (化学種拡散+エネルギー結合) は後続 |
