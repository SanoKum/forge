...
| [architecture-axisym-axis-singularity.md](architecture-axisym-axis-singularity.md) | architecture | in_progress | [`docs/turbulence/`](../../docs/turbulence/) | 軸対称「軸中心 k スパイク」の**根本原因を確定**: **float32 陰解法 (block-DPLUR) が近軸第一セルの平均速度 `u_r` を収束させきれない** (explicit float も global double も正しい)。SU2 クロスチェックと精度切り分けで、フープ/KL/dilatation は無関係 (マスク)、部分倍精度 (solve/Jac/残差蓄積/DPLUR反復)・cfl・nStepInner・scalar DPLUR は全て無効、**状態の倍精度のみ有効**と実証。根治は混合精度 (→precision-mixed-axisym) |
| [precision-mixed-axisym.md](precision-mixed-axisym.md) | precision | draft | [`docs/time_integration/`](../../docs/time_integration/) | 上記の根治。`papers/precision/itref_phys.pdf` (Baboulin 混合精度 iterative refinement) に沿い「**状態 U と残差 R は double、DPLUR 線形 solve は float**」で近軸を直す。消費者GPUでは double-float(compensated)も検討。新セッション用プロンプト: [precision-mixed-axisym-session-prompt.md](precision-mixed-axisym-session-prompt.md) |
