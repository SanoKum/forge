...
| [architecture-axisym-axis-singularity.md](architecture-axisym-axis-singularity.md) | architecture | in_progress | [`docs/turbulence/`](../../docs/turbulence/) | 軸対称近軸の数値不安定を特定。**二層**: (a) 実用格子の軽度軸 k スパイク=生産駆動→**Kato–Launder で解決** (k17→1.9, 推力不変)。(b) 軸を ≤10µm に細分すると**平均流 Uy が近軸チェッカーボード** (±7 m/s 偶奇振動, 半径方向の深い低 Mach で風上の圧力-速度結合が消失) → k 爆発。lowMachPrecond 1/2 失敗。指針: katoLaunder:1 + 軸過細分回避。根治 (near-axis 限定の圧力-速度結合回復) は将来 |
