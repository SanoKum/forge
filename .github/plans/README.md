...
| [architecture-axisym-axis-singularity.md](architecture-axisym-axis-singularity.md) | architecture | done | [`docs/turbulence/`](../../docs/turbulence/) | 軸対称「軸中心 k スパイク」の**根本原因を確定**: **float32 陰解法 (block-DPLUR) が近軸第一セルの平均速度 `u_r` を収束させきれない** (explicit float も global double も正しい)。SU2 クロスチェックと精度切り分けで、フープ/KL/dilatation は無関係 (マスク)。**追試で真因を「線形 solve の精度」と特定** (double 残差+float solve は固着、float 残差+double solve は直る)。根治は混合精度フラグ `implicitSolvePrecision` で実装 (→precision-mixed-axisym §13) |
| [precision-mixed-axisym.md](precision-mixed-axisym.md) | precision | **done** | [`docs/time_integration/`](../../docs/time_integration/) | 上記の根治。**真因は block-DPLUR 線形 solve の精度** (残差は float で可。itref の「状態+残差 double / solve float」とは逆と実証)。実装: (1) 閉形式 FVS (R/L 固有分解を排除し rank-2 外積) を既定化し float 陰解法を ~10% 高速化 (軸対称で legacy 等価)、(2) `implicitSolvePrecision` フラグ (0=float / 1=**線形 solve のみ double**) で近軸固着を根治 (laminar Uy −0.6→−14.98, ~×2.8) + 軸対称陰解の発散も安定化 (case27)。compensated double-float は ~48bit で精度不足→棄却。SST k スパイクは縮小 (9.16→7.4) するが残留 (乱流近軸の第二寄与)。詳細 §9–§13 |
