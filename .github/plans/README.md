...
| [architecture-axisym-faceweight.md](architecture-axisym-faceweight.md) | architecture | draft | [`docs/turbulence/`](../../docs/turbulence/) | 軸対称 近軸 k/ω 数値特異点の調査。case 29 で**軸中心 k がグリッド細分で発散** (17→836→1956)、背景/BL は収束、3D 非再現、平均流平滑=数値特異点と確定。当初 planar 面積仮説は誤り (flux 面積・体積は両方 r 重み済) と訂正。候補: 勾配が planar 面積使用/近軸 stiffness+生産正帰還。機構特定→根治は継続 |
