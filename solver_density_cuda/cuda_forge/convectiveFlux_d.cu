...
)
{
    ...
    if (ip < nNormalPlanes) {
        ...
        if (ip >= nNormalPlanes or conv_scheme == zero) conv_scheme = -1; // ghost
        ...
        flow_float U_R = ((Ux_R)*sxx +(Uy_R)*syy +(Uz_R)*szz)/sss;
        flow_float Ht_R= roe_R/ro_R + P_R/ro_R;
        flow_float ca_R  = sqrt(ga*P_R/ro_R);
        ...
        if (false) {
            ...
            flow_float res_ro_temp   = mdot                                        + res_ro_temp;
            flow_float res_roe_temp  = mdot*0.5*(Ht_L + Ht_R)                      + res_roe_temp;
            flow_float res_roUx_temp = mdot*0.5*(Ux_L + Ux_R) +0.5*(P_L + P_R)*sxx + res_roUx_temp;
            flow_float res_roUy_temp = mdot*0.5*(Uy_L + Uy_R) +0.5*(P_L + P_R)*syy + res_roUy_temp;
            flow_float res_roUz_temp = mdot*0.5*(Uz_L + Uz_R) +0.5*(P_L + P_R)*szz + res_roUz_temp;
        }
...
)
{
    ...
    if (ip < nNormalPlanes) {
        ...
        if (true) {
            ...
            res_ro_temp   = Ctilde*sss                      -0.5*duc*difQ2[0]*sss;
            res_roe_temp  = (Ktilde + Itilde + Ptilde)*sss  -0.5*duc*difQ2[1]*sss;
            res_roUx_temp = ((Mtildex + Gtildex)*sss)       -0.5*duc*difQ2[2]*sss;
            res_roUy_temp = ((Mtildey + Gtildey)*sss)       -0.5*duc*difQ2[3]*sss;
            res_roUz_temp = ((Mtildez + Gtildez)*sss)       -0.5*duc*difQ2[4]*sss;
...
void convectiveFlux_d_wrapper(solverConfig& cfg , cudaConfig& cuda_cfg , mesh& msh , variables& var , matrix& mat_ns)
{
    ...
    if (cfg.solver == "SLAU") {
        ...
        ROE_d<<<cuda_cfg.dimGrid_nplane , cuda_cfg.dimBlock>>> ( 
            cfg.convMethod, cfg.limiter,
            cfg.gamma,
        ...
        //KEEP_FVS_d<<<cuda_cfg.dimGrid_plane , cuda_cfg.dimBlock>>> ( 
        KEEP_FVS_d<<<cuda_cfg.dimGrid_nplane , cuda_cfg.dimBlock>>> ( 
            cfg.convMethod, cfg.limiter,
            cfg.gamma,
