...
)
{
    ...
    if (ib < nb) {
        ...
        flow_float roc = ro[ic];
        flow_float Uxc = Ux[ic];
        flow_float Uyc = Uy[ic];
        flow_float Uzc = Uz[ic];
        flow_float Umagc = sqrt(Uxc*Uxc+Uyc*Uyc+Uzc*Uzc);
        flow_float roec = roe[ic];
        flow_float Umag_new;

        //if (Uxc*sxx+Uyc*syy+Uzc*szz > 0) {
        //    Umag_new = sqrt((roec-Pnew/(ga-1.0))*2.0/roc);
        //} else {
        //    Umag_new = 0.0;
        //}
        ...
        //ro[ig]    = roc;
        //P[ig]     = Pnew;
        //Ux[ig]    = Uxc*Umag_new/Umagc;
        //Uy[ig]    = Uyc*Umag_new/Umagc;
        //Uz[ig]    = Uzc*Umag_new/Umagc;
        //roUx[ig]  = roc*Uxc*Umag_new/Umagc;
        //roUy[ig]  = roc*Uyc*Umag_new/Umagc;
        //roUz[ig]  = roc*Uzc*Umag_new/Umagc;
        //roe[ig]   = roec;
        //Ht[ig]    = roe[ig]/roc + Pnew/roc;
        //sonic[ig] = sqrt(ga*Pnew/roc);

        flow_float Un = (Uxc*sxx+Uyc*syy+Uzc*szz)/sss;
