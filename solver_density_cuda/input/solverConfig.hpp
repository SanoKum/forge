...
public:
    ...
    int keepDissJump = 0;          // 散逸ジャンプ: 0=生 (既定・ビット不変) / 1=再構成後ジャンプ (matrix CPG 枝のみ)。
                                   // 市松 (2Δ) は中心勾配ゼロで再構成が効かずフル減衰のまま、解像スケールの
                                   // ドレインを桁減 (plans/active/convection-keep-diss-recon-jump.md)。
