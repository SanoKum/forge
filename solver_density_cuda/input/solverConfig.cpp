...
//
    try {
        ...
        if (physProp["chemistry"]) {
            ...
            this->chemJacobianInterval = getOptionalValidatedValue<int>(ch, "jacobianInterval", 1, "physProp.chemistry");
            this->chemTci           = getOptionalValidatedValue<int>(ch, "tci", 0, "physProp.chemistry");
            ...
            if (ch["cmc"]) {
                ...
                this->chemCmcTauRelax = getOptionalValidatedValue<double>(cm, "tauRelax", 1.0e-4, "physProp.chemistry.cmc");
                ...
                if (this->chemCmc) {
                    ...
                    if (this->chemCmcCouple < 0 || this->chemCmcCouple > 7) throw std::runtime_error("'physProp.chemistry.cmc.couple' must be 0 (passive), 1 (PDF-mean source), 2 (relaxation), 3 (blend Y,h), 4 (blend Y + conditional reaction heat added to roe), 5 (same heat via the implicit chemistry source path), 6 (blend Y + relax mean h to h_pdf in the burning region via the implicit path), or 7 (physical-time relaxation sources with cmc.tauRelax; use for dual-time).");
