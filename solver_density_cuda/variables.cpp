...
void variables::registerMixfrac()
{
    ...
    for (const auto& name : {std::string("cmc_dY"), std::string("cmc_TQmax"), std::string("cmc_TQst"), std::string("cmc_xiOm"), std::string("cmc_xipdf")}) {
        this->cellValNames.push_back(name);
        this->c.emplace(name, std::vector<flow_float>{});
        this->c_d.emplace(name, nullptr);
    }
    for (const auto& name : {std::string("roXiVar"), std::string("xiVar"), std::string("xi"), std::string("chi"), std::string("cmc_dY"), std::string("cmc_TQmax"), std::string("cmc_TQst"), std::string("cmc_xiOm"), std::string("cmc_xipdf")})
        this->output_cellValNames.push_back(name);
