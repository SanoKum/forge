...
void variables::registerMixfrac()
{
    ...
    for (const auto& name : {std::string("cmc_dY"), std::string("cmc_TQmax"), std::string("cmc_TQst")}) {
        this->cellValNames.push_back(name);
        this->c.emplace(name, std::vector<flow_float>{});
        this->c_d.emplace(name, nullptr);
    }
    for (const auto& name : {std::string("roXiVar"), std::string("xiVar"), std::string("xi"), std::string("chi"), std::string("cmc_dY"), std::string("cmc_TQmax"), std::string("cmc_TQst")})
        this->output_cellValNames.push_back(name);
